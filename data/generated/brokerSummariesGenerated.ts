/**
 * Generated Broker Summaries
 *
 * This file is auto-generated by scripts/generateBrokerSummaries.ts
 * Do not edit manually - run the script instead.
 *
 * Generated on: 2025-10-15T07:14:19.895Z
 */

import type { BrokerSummary } from '../brokerSummaries'

export const brokerSummaries: BrokerSummary[] = [
  {
    id: 'pepperstone',
    name: 'Pepperstone',
    logoUrl: '/broker-logos/pepperstone.png',
    websiteUrl: 'https://pepperstone.com/',
    score: 9.2,
    foundingYear: 2010,
    headquarters: 'Melbourne, Australia',
    description: 'Pepperstone is an award-winning online broker known for its fast execution, low spreads, and a wide range of trading platforms. It is regulated by top-tier authorities like ASIC and the FCA.',
    rating: 9.8,
    regulation: [
      'ASIC',
      'FCA',
      'CySEC',
      'DFSA',
      'BaFin',
      'CMA',
    ],
    minDeposit: 0,
    spreads: {
      eurusd: 0.06,
      gbpusd: 0.16,
      usdjpy: 0.09,
    },
    accessibility: {
      minDeposit: 0,
      depositMethods: [
        'Credit Card',
        'Debit Card',
        'Bank Transfer',
        'PayPal',
        'Skrill',
        'Neteller',
      ],
      withdrawalMethods: [
        'Credit Card',
        'Bank Transfer',
        'PayPal',
        'Skrill',
      ],
      customerSupport: [
        '24/5 Live Chat',
        'Phone',
        'Email',
      ],
    },
    brokerType: 'ECN',
    platforms: [
      'MT4',
      'MT5',
      'cTrader',
      'TradingView',
    ],
    countries: [
      'Global',
      'Global',
      'Global',
      'Global',
      'Global',
      'Global',
    ],
    features: [
      'EA Support',
      'API Access',
      'Islamic Account',
      'Copy Trading',
      'Islamic Friendly',
    ],
  },
  {
    id: 'ic-markets',
    name: 'IC Markets',
    logoUrl: '/broker-logos/ic-markets.jpg',
    websiteUrl: 'https://www.icmarkets.com/',
    score: 9,
    foundingYear: 2007,
    headquarters: 'Sydney, Australia',
    description: 'IC Markets is one of the most renowned Forex CFD providers, offering trading solutions for active day traders and scalpers as well as traders that are new to the forex market.',
    rating: 9.5,
    regulation: [
      'ASIC',
      'CySEC',
      'FSA',
    ],
    minDeposit: 200,
    spreads: {
      eurusd: 0,
      gbpusd: 0.2,
      usdjpy: 0.1,
    },
    accessibility: {
      minDeposit: 200,
      depositMethods: [
        'Credit Card',
        'Bank Transfer',
        'PayPal',
        'Neteller',
      ],
      withdrawalMethods: [
        'Credit Card',
        'Bank Transfer',
        'PayPal',
      ],
      customerSupport: [
        '24/7 Live Chat',
        'Phone',
        'Email',
      ],
    },
    brokerType: 'ECN',
    platforms: [
      'MT4',
      'MT5',
      'cTrader',
    ],
    countries: [
      'Global',
      'Global',
      'Global',
    ],
    features: [
      'EA Support',
      'API Access',
      'Islamic Account',
      'Copy Trading',
      'Islamic Friendly',
    ],
  },
  {
    id: 'xtb',
    name: 'XTB',
    logoUrl: '/broker-logos/xtb.png',
    websiteUrl: 'https://www.xtb.com/',
    score: 9.5,
    foundingYear: 2002,
    headquarters: 'Warsaw, Poland',
    description: 'X-Trade Brokers Dom Maklerski S.A. (WSE: XTB) is a publicly-traded, award-winning forex and CFD broker serving 500,000+ clients across 13+ countries. Founded in 2002, XTB revolutionized trading with i...',
    rating: 9.2,
    regulation: [
      'FCA',
      'KNF',
      'CySEC',
      'IFSC',
    ],
    minDeposit: 0,
    spreads: {
      eurusd: 0.5,
      gbpusd: 0.8,
      usdjpy: 0.6,
    },
    accessibility: {
      minDeposit: 0,
      depositMethods: [
        'Bank Transfer',
        'Credit Card',
        'Skrill',
      ],
      withdrawalMethods: [
        'Bank Transfer',
        'Credit Card',
      ],
      customerSupport: [
        '24/5 Live Chat',
        'Phone',
        'Email',
      ],
    },
    brokerType: 'Market Maker',
    platforms: [
      'xStation 5',
    ],
    countries: [
      'Global',
      'Global',
      'Global',
      'Global',
    ],
    features: [
      'Islamic Account',
      'Islamic Friendly',
    ],
  },
  {
    id: 'forex-com',
    name: 'Forex.com',
    logoUrl: '/broker-logos/forex-com.png',
    websiteUrl: 'https://www.forex.com/',
    score: 9.2,
    foundingYear: 2001,
    headquarters: 'New Jersey, USA',
    description: 'FOREX.com, a division of StoneX Group Inc. (NASDAQ: SNEX), is the US regulatory compliance leader offering professional trading since 2001. As a publicly-traded company with comprehensive US regulatio...',
    rating: 9.9,
    regulation: [
      'FCA',
      'NFA',
      'ASIC',
      'CIMA',
    ],
    minDeposit: 100,
    spreads: {
      eurusd: 1,
      gbpusd: 1.5,
      usdjpy: 1.2,
    },
    accessibility: {
      minDeposit: 100,
      depositMethods: [
        'Credit Card',
        'Debit Card',
        'Bank Transfer',
      ],
      withdrawalMethods: [
        'Credit Card',
        'Bank Transfer',
      ],
      customerSupport: [
        '24/5 Phone',
        'Email',
      ],
    },
    brokerType: 'Market Maker',
    platforms: [
      'MT4',
      'Advanced Trading Platform',
      'TradingView',
    ],
    countries: [
      'Global',
      'Global',
      'Global',
      'Global',
    ],
    features: [
      'EA Support',
      'API Access',
    ],
  },
  {
    id: 'ig',
    name: 'IG',
    logoUrl: '/broker-logos/ig.png',
    websiteUrl: 'https://www.ig.com/',
    score: 9.5,
    foundingYear: 1974,
    headquarters: 'The Leadenhall Building, 122 Leadenhall Street, London EC3V 4AB, UK',
    description: 'IG Group Holdings plc is a global leader in online trading, providing access to over 17,000 financial markets across 18+ countries. Listed on the London Stock Exchange (LSE: IGG) and serving 300,000+ ...',
    rating: 9.9,
    regulation: [
      'FCA',
      'ASIC',
      'CySEC',
      'NFA',
      'FSCA',
    ],
    minDeposit: 250,
    spreads: {
      eurusd: 0.6,
      gbpusd: 0.9,
      usdjpy: 0.7,
    },
    accessibility: {
      minDeposit: 250,
      depositMethods: [
        'Credit Card',
        'Bank Transfer',
        'PayPal',
      ],
      withdrawalMethods: [
        'Credit Card',
        'Bank Transfer',
        'PayPal',
      ],
      customerSupport: [
        '24/5 Live Chat',
        'Phone',
        'Email',
      ],
    },
    brokerType: 'Market Maker',
    platforms: [
      'IG Trading Platform (Web & Mobile)',
      'ProRealTime',
      'MT4',
      'RESTful API',
    ],
    countries: [
      'Global',
      'Global',
      'Global',
      'Global',
      'Global',
    ],
    features: [
      'EA Support',
      'API Access',
      'Islamic Account',
      'Copy Trading',
      'Islamic Friendly',
      'Trading Signals',
    ],
  },
  {
    id: 'saxo-bank',
    name: 'Saxo Bank',
    logoUrl: '/broker-logos/saxo-bank.png',
    websiteUrl: 'https://www.home.saxo/',
    score: 9.4,
    foundingYear: 1992,
    headquarters: 'Philip Heymans Allé 15, 2900 Hellerup, Denmark',
    description: 'Saxo Bank A/S is a fully licensed Danish investment bank offering premium trading across 71,000+ instruments. Serving 1M+ clients in 180+ countries with $90B+ client assets under management.',
    rating: 9.8,
    regulation: [
      'DFSA',
      'FCA',
      'ASIC',
      'MAS',
      'FINMA',
    ],
    minDeposit: 2000,
    spreads: {
      eurusd: 0.7,
      gbpusd: 1,
      usdjpy: 0.8,
    },
    accessibility: {
      minDeposit: 2000,
      depositMethods: [
        'Bank Transfer',
        'Credit Card',
      ],
      withdrawalMethods: [
        'Bank Transfer',
      ],
      customerSupport: [
        '24/5 Phone',
        'Email',
      ],
    },
    brokerType: 'Hybrid',
    platforms: [
      'SaxoTraderGO',
      'SaxoTraderPRO',
    ],
    countries: [
      'Global',
      'Global',
      'Global',
      'Global',
      'Global',
    ],
    features: [
      'API Access',
      'Trading Signals',
    ],
  },
  {
    id: 'interactive-brokers',
    name: 'Interactive Brokers',
    logoUrl: '/broker-logos/interactive-brokers.png',
    websiteUrl: 'https://www.interactivebrokers.com/',
    score: 9.3,
    foundingYear: 1978,
    headquarters: 'Greenwich, Connecticut, USA',
    description: 'A top choice for professional and institutional traders, offering unparalleled market access, low costs, and sophisticated trading tools.',
    rating: 5,
    regulation: [
      'SEC',
      'FINRA',
      'CFTC',
      'FCA',
      'IIROC',
    ],
    minDeposit: 0,
    spreads: {
      eurusd: 0.2,
      gbpusd: 0.5,
      usdjpy: 0.2,
    },
    accessibility: {
      minDeposit: 0,
      depositMethods: [
        'Bank Transfer',
      ],
      withdrawalMethods: [
        'Bank Transfer',
      ],
      customerSupport: [
        'Phone',
        'Email',
        'Chat',
      ],
    },
    brokerType: 'ECN',
    platforms: [
      'TWS (Trader Workstation)',
      'IBKR Mobile',
      'WebTrader',
      'API',
    ],
    countries: [
      'Global',
      'Global',
      'Global',
      'Global',
      'Global',
    ],
    features: [
      'EA Support',
      'API Access',
    ],
  },
  {
    id: 'etoro',
    name: 'eToro',
    logoUrl: '/broker-logos/etoro.png',
    websiteUrl: 'https://www.etoro.com/',
    score: 8.9,
    foundingYear: 2007,
    headquarters: 'Tel Aviv, Israel & London, UK',
    description: "eToro is the world's leading social trading platform, offering a wide array of stocks, cryptocurrencies, and CFDs. It is well-regulated and known for its user-friendly interface.",
    rating: 9,
    regulation: [
      'FCA',
      'ASIC',
      'CySEC',
    ],
    minDeposit: 50,
    spreads: {
      eurusd: 1,
      gbpusd: 1.5,
      usdjpy: 1.2,
    },
    accessibility: {
      minDeposit: 50,
      depositMethods: [
        'Credit Card',
        'PayPal',
        'Skrill',
      ],
      withdrawalMethods: [
        'Credit Card',
        'PayPal',
      ],
      customerSupport: [
        'Live Chat',
      ],
    },
    brokerType: 'Market Maker',
    platforms: [
      'eToro Platform',
    ],
    countries: [
      'Global',
      'Global',
      'Global',
    ],
    features: [
      'Islamic Account',
      'Copy Trading',
      'Islamic Friendly',
      'Trading Signals',
    ],
  },
  {
    id: 'plus500',
    name: 'Plus500',
    logoUrl: '/broker-logos/plus500.png',
    websiteUrl: 'https://www.plus500.com/',
    score: 8.4,
    foundingYear: 2008,
    headquarters: 'Haifa, Israel & London, UK',
    description: 'Plus500 is a global CFD provider, listed on the London Stock Exchange. It offers a simple, user-friendly platform with a wide range of tradable instruments.',
    rating: 9,
    regulation: [
      'FCA',
      'ASIC',
      'CySEC',
    ],
    minDeposit: 100,
    spreads: {
      eurusd: 0.6,
      gbpusd: 1.2,
      usdjpy: 0.9,
    },
    accessibility: {
      minDeposit: 100,
      depositMethods: [
        'Credit Card',
        'PayPal',
        'Skrill',
      ],
      withdrawalMethods: [
        'Credit Card',
        'PayPal',
      ],
      customerSupport: [
        '24/7 Live Chat',
        'Email',
      ],
    },
    brokerType: 'Market Maker',
    platforms: [
      'Plus500 Platform',
    ],
    countries: [
      'Global',
      'Global',
      'Global',
    ],
    features: [],
  },
  {
    id: 'avatrade',
    name: 'AvaTrade',
    logoUrl: '/broker-logos/avatrade.webp',
    websiteUrl: 'https://www.avatrade.com/',
    score: 8.7,
    foundingYear: 2006,
    headquarters: 'Dublin, Ireland',
    description: 'AvaTrade is a highly-regulated global broker known for its extensive range of trading platforms, including its proprietary AvaTradeGo app, and its comprehensive educational resources.',
    rating: 9.3,
    regulation: [
      'CBI',
      'ASIC',
      'FSA',
      'FSCA',
    ],
    minDeposit: 100,
    spreads: {
      eurusd: 0.9,
      gbpusd: 1.5,
      usdjpy: 1.1,
    },
    accessibility: {
      minDeposit: 100,
      depositMethods: [
        'Credit Card',
        'Skrill',
        'Neteller',
      ],
      withdrawalMethods: [
        'Credit Card',
        'Skrill',
      ],
      customerSupport: [
        '24/5 Live Chat',
        'Phone',
      ],
    },
    brokerType: 'Market Maker',
    platforms: [
      'MT4',
      'MT5',
      'AvaTradeGo',
      'AvaOptions',
    ],
    countries: [
      'Global',
      'Global',
      'Global',
      'Global',
    ],
    features: [
      'EA Support',
      'API Access',
      'Islamic Account',
      'Copy Trading',
      'Islamic Friendly',
      'Trading Signals',
    ],
  },
  {
    id: 'oanda',
    name: 'OANDA',
    logoUrl: '/broker-logos/oanda.png',
    websiteUrl: 'https://www.oanda.com/',
    score: 8.6,
    foundingYear: 1996,
    headquarters: '1 Pickering Street #26-02, Singapore 048659 (Global HQ)',
    description: 'OANDA Corporation is the pioneer in online FX trading and API technology, with a 28-year history of innovation in fractional pricing and institutional-grade execution. Managing $2B+ in client assets a...',
    rating: 9.5,
    regulation: [
      'NFA',
      'FCA',
      'ASIC',
    ],
    minDeposit: 0,
    spreads: {
      eurusd: 0.6,
      gbpusd: 0.9,
      usdjpy: 0.7,
    },
    accessibility: {
      minDeposit: 0,
      depositMethods: [
        'Credit Card',
        'PayPal',
      ],
      withdrawalMethods: [
        'Credit Card',
        'PayPal',
      ],
      customerSupport: [
        '24/5 Live Chat',
        'Phone',
      ],
    },
    brokerType: 'Market Maker',
    platforms: [
      'fxTrade',
      'MT4',
      'TradingView',
    ],
    countries: [
      'Global',
      'Global',
      'Global',
    ],
    features: [
      'EA Support',
      'API Access',
      'Trading Signals',
    ],
  },
  {
    id: 'fxpro',
    name: 'FxPro',
    logoUrl: '/broker-logos/fxpro.png',
    websiteUrl: 'https://www.fxpro.com/',
    score: 9.4,
    foundingYear: 2006,
    headquarters: 'London, UK',
    description: 'FxPro is a globally recognized institutional-grade broker serving 1.3+ million clients worldwide. Founded in 2006, FxPro pioneered the No Dealing Desk (NDD) execution model and offers professional tra...',
    rating: 9.2,
    regulation: [
      'FCA',
      'CySEC',
      'FSCA',
      'SCB',
    ],
    minDeposit: 100,
    spreads: {
      eurusd: 1.2,
      gbpusd: 1.5,
      usdjpy: 1.4,
    },
    accessibility: {
      minDeposit: 100,
      depositMethods: [
        'Credit Card',
        'PayPal',
        'Skrill',
      ],
      withdrawalMethods: [
        'Credit Card',
        'PayPal',
        'Skrill',
      ],
      customerSupport: [
        '24/5 Live Chat',
        'Phone',
      ],
    },
    brokerType: 'NDD',
    platforms: [
      'MT4',
      'MT5',
      'cTrader',
      'FxPro Edge',
    ],
    countries: [
      'Global',
      'Global',
      'Global',
      'Global',
    ],
    features: [
      'EA Support',
      'Islamic Account',
      'Islamic Friendly',
    ],
  },
  {
    id: 'axi',
    name: 'Axi',
    logoUrl: '/broker-logos/axi.png',
    websiteUrl: 'https://www.axi.com/',
    score: 8.7,
    foundingYear: 2007,
    headquarters: 'Sydney, Australia',
    description: 'Axi (formerly AxiTrader) is an Australian broker trusted by traders for its excellent customer service, competitive pricing, and strong regulatory oversight.',
    rating: 9,
    regulation: [
      'ASIC',
      'FCA',
      'DFSA',
    ],
    minDeposit: 0,
    spreads: {
      eurusd: 1.2,
      gbpusd: 1.6,
      usdjpy: 1.4,
    },
    accessibility: {
      minDeposit: 0,
      depositMethods: [
        'Credit Card',
        'Bank Transfer',
        'Skrill',
      ],
      withdrawalMethods: [
        'Credit Card',
        'Bank Transfer',
      ],
      customerSupport: [
        '24/5 Live Chat',
        'Phone',
      ],
    },
    brokerType: 'STP',
    platforms: [
      'MT4',
    ],
    countries: [
      'Global',
      'Global',
      'Global',
    ],
    features: [
      'EA Support',
      'Islamic Account',
      'Copy Trading',
      'Islamic Friendly',
      'Trading Signals',
    ],
  },
  {
    id: 'fp-markets',
    name: 'FP Markets',
    logoUrl: '/broker-logos/fp-markets.webp',
    websiteUrl: 'https://www.fpmarkets.com/',
    score: 8.9,
    foundingYear: 2005,
    headquarters: 'Sydney, Australia',
    description: 'FP Markets is an Australian-regulated broker offering consistently tight raw spreads and a broad range of tradable instruments across multiple platforms.',
    rating: 9,
    regulation: [
      'ASIC',
      'CySEC',
    ],
    minDeposit: 100,
    spreads: {
      eurusd: 1.2,
      gbpusd: 1.5,
      usdjpy: 1.3,
    },
    accessibility: {
      minDeposit: 100,
      depositMethods: [
        'Credit Card',
        'Bank Transfer',
        'PayPal',
      ],
      withdrawalMethods: [
        'Credit Card',
        'Bank Transfer',
        'PayPal',
      ],
      customerSupport: [
        '24/7 Live Chat',
        'Phone',
      ],
    },
    brokerType: 'ECN',
    platforms: [
      'MT4',
      'MT5',
      'cTrader',
      'Iress',
    ],
    countries: [
      'Global',
      'Global',
    ],
    features: [
      'EA Support',
      'API Access',
      'Islamic Account',
      'Copy Trading',
      'Islamic Friendly',
    ],
  },
  {
    id: 'cmc-markets',
    name: 'CMC Markets',
    logoUrl: 'https://www.cmcmarkets.com/logo-cmc-markets.svg',
    websiteUrl: 'https://www.cmcmarkets.com/',
    score: 9.6,
    foundingYear: 1989,
    headquarters: 'London, UK',
    description: 'CMC Markets Group plc (LSE: CMCX) is a globally recognized, publicly-listed financial services provider offering CFD and spread betting through its award-winning "Next Generation" platform. With 35+ y...',
    rating: 9.8,
    regulation: [
      'FCA',
      'ASIC',
      'BaFin',
    ],
    minDeposit: 0,
    spreads: {
      eurusd: 0.7,
      gbpusd: 0.9,
      usdjpy: 0.7,
    },
    accessibility: {
      minDeposit: 0,
      depositMethods: [
        'Credit Card',
        'Bank Transfer',
        'PayPal',
      ],
      withdrawalMethods: [
        'Credit Card',
        'Bank Transfer',
        'PayPal',
      ],
      customerSupport: [
        '24/5 Live Chat',
        'Phone',
      ],
    },
    brokerType: 'Market Maker',
    platforms: [
      'Next Generation',
      'MT4',
    ],
    countries: [
      'Global',
      'Global',
      'Global',
    ],
    features: [
      'EA Support',
      'API Access',
      'Trading Signals',
    ],
  },
  {
    id: 'admirals',
    name: 'Admirals',
    logoUrl: '/broker-logos/admirals.png',
    websiteUrl: 'https://admiralmarkets.com/',
    score: 9.3,
    foundingYear: 2001,
    headquarters: 'Tallinn, Estonia',
    description: 'Admirals (formerly Admiral Markets) is an award-winning educational leader and MetaTrader authority serving traders globally since 2001. With tier-1 regulation and the exclusive MetaTrader Supreme Edi...',
    rating: 9,
    regulation: [
      'FCA',
      'ASIC',
      'CySEC',
    ],
    minDeposit: 25,
    spreads: {
      eurusd: 0.6,
      gbpusd: 0.8,
      usdjpy: 0.7,
    },
    accessibility: {
      minDeposit: 25,
      depositMethods: [
        'Credit Card',
        'Bank Transfer',
        'Skrill',
      ],
      withdrawalMethods: [
        'Bank Transfer',
      ],
      customerSupport: [
        '24/5 Live Chat',
        'Phone',
      ],
    },
    brokerType: 'STP',
    platforms: [
      'MT4',
      'MT5',
      'MetaTrader Supreme Edition',
    ],
    countries: [
      'Global',
      'Global',
      'Global',
    ],
    features: [
      'EA Support',
      'Islamic Account',
      'Copy Trading',
      'Islamic Friendly',
      'Trading Signals',
    ],
  },
  {
    id: 'tickmill',
    name: 'Tickmill',
    logoUrl: '/broker-logos/tickmill.png',
    websiteUrl: 'https://www.tickmill.com/',
    score: 8.8,
    foundingYear: 2014,
    headquarters: 'London, UK',
    description: 'Tickmill is a popular MetaTrader broker known for its very low commissions on Pro and VIP accounts, fast execution, and strong regulatory framework.',
    rating: 9,
    regulation: [
      'FCA',
      'CySEC',
      'FSA',
    ],
    minDeposit: 100,
    spreads: {
      eurusd: 0.1,
      gbpusd: 0.2,
      usdjpy: 0.1,
    },
    accessibility: {
      minDeposit: 100,
      depositMethods: [
        'Credit Card',
        'Bank Transfer',
        'Skrill',
      ],
      withdrawalMethods: [
        'Credit Card',
        'Bank Transfer',
      ],
      customerSupport: [
        '24/5 Live Chat',
        'Phone',
      ],
    },
    brokerType: 'ECN',
    platforms: [
      'MT4',
      'MT5',
    ],
    countries: [
      'Global',
      'Global',
      'Global',
    ],
    features: [
      'EA Support',
      'Islamic Account',
      'Copy Trading',
      'Islamic Friendly',
    ],
  },
  {
    id: 'swissquote',
    name: 'Swissquote',
    logoUrl: '/broker-logos/swissquote.png',
    websiteUrl: 'https://www.swissquote.com/',
    score: 9.7,
    foundingYear: 1996,
    headquarters: 'Gland, Switzerland',
    description: "Swissquote Bank Ltd (SIX: SQN) is a publicly-listed Swiss banking institution offering premium financial services since 1996. As Switzerland's leading online bank with FINMA regulation, Swissquote com...",
    rating: 9.9,
    regulation: [
      'FINMA',
      'FCA',
      'DFSA',
    ],
    minDeposit: 1000,
    spreads: {
      eurusd: 1.7,
      gbpusd: 2,
      usdjpy: 1.8,
    },
    accessibility: {
      minDeposit: 1000,
      depositMethods: [
        'Bank Transfer',
        'Credit Card',
      ],
      withdrawalMethods: [
        'Bank Transfer',
      ],
      customerSupport: [
        '24/5 Phone',
        'Email',
      ],
    },
    brokerType: 'STP',
    platforms: [
      'Advanced Trader',
      'MT4',
      'MT5',
    ],
    countries: [
      'Global',
      'Global',
      'Global',
    ],
    features: [
      'EA Support',
      'API Access',
      'Copy Trading',
      'Trading Signals',
    ],
  },
  {
    id: 'dukascopy',
    name: 'Dukascopy',
    logoUrl: '/broker-logos/dukascopy.png',
    websiteUrl: 'https://www.dukascopy.com/',
    score: 9.5,
    foundingYear: 2004,
    headquarters: 'Geneva, Switzerland',
    description: "Dukascopy Bank SA is a Swiss banking institution pioneering the SWFX (Swiss FX Marketplace) - the world's first ECN marketplace. With FINMA banking license and proprietary JForex platform technology, ...",
    rating: 9.9,
    regulation: [
      'FINMA',
      'FSA (Japan)',
    ],
    minDeposit: 100,
    spreads: {
      eurusd: 0.2,
      gbpusd: 0.5,
      usdjpy: 0.3,
    },
    accessibility: {
      minDeposit: 100,
      depositMethods: [
        'Bank Transfer',
        'Credit Card',
      ],
      withdrawalMethods: [
        'Bank Transfer',
      ],
      customerSupport: [
        '24/5 Phone',
        'Email',
      ],
    },
    brokerType: 'ECN',
    platforms: [
      'JForex',
      'MT4',
    ],
    countries: [
      'Global',
      'Global',
    ],
    features: [
      'EA Support',
      'API Access',
      'Islamic Account',
      'Copy Trading',
      'Islamic Friendly',
      'Trading Signals',
    ],
  },
  {
    id: 'thinkmarkets',
    name: 'ThinkMarkets',
    logoUrl: '/broker-logos/thinkmarkets.png',
    websiteUrl: 'https://www.thinkmarkets.com/',
    score: 9.3,
    foundingYear: 2010,
    headquarters: 'Melbourne, Australia & London, UK',
    description: 'ThinkMarkets Global Pty Ltd is the proprietary platform innovation leader offering the award-winning ThinkTrader technology since 2010. With dual tier-1 regulation (FCA/ASIC) and zero minimum deposit ...',
    rating: 9.2,
    regulation: [
      'FCA',
      'ASIC',
      'FSCA',
    ],
    minDeposit: 0,
    spreads: {
      eurusd: 1.1,
      gbpusd: 1.4,
      usdjpy: 1.2,
    },
    accessibility: {
      minDeposit: 0,
      depositMethods: [
        'Credit Card',
        'Skrill',
        'PayPal',
      ],
      withdrawalMethods: [
        'Credit Card',
        'Skrill',
        'PayPal',
      ],
      customerSupport: [
        '24/5 Live Chat',
        'Phone',
      ],
    },
    brokerType: 'STP',
    platforms: [
      'ThinkTrader',
      'MT4',
      'MT5',
    ],
    countries: [
      'Global',
      'Global',
      'Global',
    ],
    features: [
      'EA Support',
      'Islamic Account',
      'Copy Trading',
      'Islamic Friendly',
      'Trading Signals',
    ],
  },
  {
    id: 'fxcm',
    name: 'FXCM',
    logoUrl: '/broker-logos/fxcm.png',
    websiteUrl: 'https://www.fxcm.com/',
    score: 8.4,
    foundingYear: 1999,
    headquarters: '20 Gresham Street, 4th Floor, London EC2V 7JE, UK',
    description: 'FXCM (Forex Capital Markets Limited), restructured in 2017 with renewed focus, offers institutional-grade infrastructure across 190+ countries. Known for Trading Station professional platform and comp...',
    rating: 9,
    regulation: [
      'FCA',
      'ASIC',
      'CySEC',
    ],
    minDeposit: 50,
    spreads: {
      eurusd: 1.3,
      gbpusd: 1.6,
      usdjpy: 1.4,
    },
    accessibility: {
      minDeposit: 50,
      depositMethods: [
        'Credit Card',
        'Bank Transfer',
        'Skrill',
      ],
      withdrawalMethods: [
        'Credit Card',
        'Bank Transfer',
      ],
      customerSupport: [
        '24/5 Live Chat',
        'Phone',
      ],
    },
    brokerType: 'STP',
    platforms: [
      'Trading Station (Professional)',
      'MetaTrader 4',
      'TradingView Integration',
    ],
    countries: [
      'Global',
      'Global',
      'Global',
    ],
    features: [
      'EA Support',
      'API Access',
      'Islamic Account',
      'Copy Trading',
      'Islamic Friendly',
      'Trading Signals',
    ],
  },
  {
    id: 'xm',
    name: 'XM',
    logoUrl: '/broker-logos/xm.png',
    websiteUrl: 'https://www.xm.com/',
    score: 8.5,
    foundingYear: 2009,
    headquarters: '12 Richard & Verengaria Street, Araouzos Castle Court, 3042 Limassol, Cyprus',
    description: 'XM Group (Trading Point Holdings Ltd) is a global leader with 5M+ clients in 190+ countries, known for its comprehensive multi-jurisdictional regulation, educational focus, and diverse account offerin...',
    rating: 8.8,
    regulation: [
      'CySEC',
      'ASIC',
      'IFSC',
    ],
    minDeposit: 5,
    spreads: {
      eurusd: 1.7,
      gbpusd: 2.5,
      usdjpy: 1.8,
    },
    accessibility: {
      minDeposit: 5,
      depositMethods: [
        'Credit Card',
        'Bank Transfer',
        'Skrill',
      ],
      withdrawalMethods: [
        'Credit Card',
        'Bank Transfer',
        'Skrill',
      ],
      customerSupport: [
        '24/5 Live Chat',
        'Phone',
      ],
    },
    brokerType: 'Market Maker',
    platforms: [
      'MT4',
      'MT5',
    ],
    countries: [
      'Global',
      'Global',
      'Global',
    ],
    features: [
      'EA Support',
      'Islamic Account',
      'Islamic Friendly',
      'Trading Signals',
    ],
  },
  {
    id: 'exness',
    name: 'Exness',
    logoUrl: '/broker-logos/exness.webp',
    websiteUrl: 'https://www.exness.com/',
    score: 8.9,
    foundingYear: 2008,
    headquarters: 'Limassol, Cyprus',
    description: 'Exness is a large retail forex broker known for its instant withdrawal system, extremely high leverage options, and transparent reporting, including tick-level data.',
    rating: 8.5,
    regulation: [
      'FCA',
      'CySEC',
      'FSCA',
    ],
    minDeposit: 10,
    spreads: {
      eurusd: 0.6,
      gbpusd: 0.8,
      usdjpy: 0.7,
    },
    accessibility: {
      minDeposit: 10,
      depositMethods: [
        'Credit Card',
        'Skrill',
        'Neteller',
      ],
      withdrawalMethods: [
        'Credit Card',
        'Skrill',
        'Neteller',
      ],
      customerSupport: [
        '24/7 Live Chat',
        'Phone',
      ],
    },
    brokerType: 'Market Maker',
    platforms: [
      'MT4',
      'MT5',
      'Exness Terminal',
    ],
    countries: [
      'Global',
      'Global',
      'Global',
    ],
    features: [
      'EA Support',
      'Islamic Account',
      'Copy Trading',
      'Islamic Friendly',
    ],
  },
  {
    id: 'hf-markets',
    name: 'HF Markets',
    logoUrl: '/broker-logos/hfmarkets.png',
    websiteUrl: 'https://www.hfm.com/',
    score: 8.6,
    foundingYear: 2010,
    headquarters: 'Kingstown, St. Vincent & the Grenadines',
    description: 'HF Markets (formerly HotForex) is a global forex and commodities broker with a strong presence in emerging markets, offering a wide range of trading accounts and assets.',
    rating: 8.9,
    regulation: [
      'FCA',
      'CySEC',
      'DFSA',
    ],
    minDeposit: 0,
    spreads: {
      eurusd: 1.3,
      gbpusd: 1.6,
      usdjpy: 1.4,
    },
    accessibility: {
      minDeposit: 0,
      depositMethods: [
        'Credit Card',
        'Bank Transfer',
        'Skrill',
      ],
      withdrawalMethods: [
        'Credit Card',
        'Bank Transfer',
        'Skrill',
      ],
      customerSupport: [
        '24/5 Live Chat',
        'Phone',
      ],
    },
    brokerType: 'STP',
    platforms: [
      'MT4',
      'MT5',
    ],
    countries: [
      'Global',
      'Global',
      'Global',
    ],
    features: [
      'EA Support',
      'Islamic Account',
      'Copy Trading',
      'Islamic Friendly',
      'Trading Signals',
    ],
  },
  {
    id: 'fbs',
    name: 'FBS',
    logoUrl: '/broker-logos/fbs.png',
    websiteUrl: 'https://fbs.com/',
    score: 8.2,
    foundingYear: 2009,
    headquarters: 'Limassol, Cyprus',
    description: 'FBS is an international broker known for offering extremely high leverage, a low minimum deposit, and a wide variety of promotions and contests, making it popular in Asia and other emerging markets.',
    rating: 7.5,
    regulation: [
      'ASIC',
      'CySEC',
      'FSCA',
    ],
    minDeposit: 1,
    spreads: {
      eurusd: 0.9,
      gbpusd: 1.2,
      usdjpy: 1.1,
    },
    accessibility: {
      minDeposit: 1,
      depositMethods: [
        'Credit Card',
        'Skrill',
        'Neteller',
      ],
      withdrawalMethods: [
        'Credit Card',
        'Skrill',
        'Neteller',
      ],
      customerSupport: [
        '24/7 Live Chat',
      ],
    },
    brokerType: 'STP',
    platforms: [
      'MT4',
      'MT5',
      'FBS Trader',
    ],
    countries: [
      'Global',
      'Global',
      'Global',
    ],
    features: [
      'EA Support',
      'Islamic Account',
      'Copy Trading',
      'Islamic Friendly',
      'Trading Signals',
    ],
  },
  {
    id: 'octafx',
    name: 'OctaFX',
    logoUrl: '/broker-logos/octafx.webp',
    websiteUrl: 'https://www.octafx.com/',
    score: 8.3,
    foundingYear: 2011,
    headquarters: 'Kingstown, St. Vincent & the Grenadines',
    description: 'OctaFX is a global broker recognized for its low-cost trading environment, user-friendly copy trading platform, and a variety of promotional offers.',
    rating: 7,
    regulation: [
      'CySEC',
    ],
    minDeposit: 25,
    spreads: {
      eurusd: 0.6,
      gbpusd: 0.8,
      usdjpy: 0.7,
    },
    accessibility: {
      minDeposit: 25,
      depositMethods: [
        'Credit Card',
        'Skrill',
        'Crypto',
      ],
      withdrawalMethods: [
        'Credit Card',
        'Skrill',
        'Crypto',
      ],
      customerSupport: [
        '24/7 Live Chat',
      ],
    },
    brokerType: 'STP',
    platforms: [
      'MT4',
      'MT5',
    ],
    countries: [
      'Global',
    ],
    features: [
      'EA Support',
      'Islamic Account',
      'Copy Trading',
      'Islamic Friendly',
      'Trading Signals',
    ],
  },
  {
    id: 'roboforex',
    name: 'RoboForex',
    logoUrl: '/broker-logos/roboforex.webp',
    websiteUrl: 'https://roboforex.com/',
    score: 8.7,
    foundingYear: 2009,
    headquarters: 'Belize City, Belize',
    description: 'RoboForex is a versatile broker offering a wide variety of account types, platforms, and assets, with a strong focus on algorithmic trading and investment platforms.',
    rating: 6.5,
    regulation: [
      'FSC Belize',
    ],
    minDeposit: 10,
    spreads: {
      eurusd: 0,
      gbpusd: 0.3,
      usdjpy: 0.1,
    },
    accessibility: {
      minDeposit: 10,
      depositMethods: [
        'Credit Card',
        'Skrill',
        'Neteller',
      ],
      withdrawalMethods: [
        'Credit Card',
        'Skrill',
        'Neteller',
      ],
      customerSupport: [
        '24/7 Live Chat',
        'Phone',
      ],
    },
    brokerType: 'Hybrid',
    platforms: [
      'MT4',
      'MT5',
      'cTrader',
      'R StocksTrader',
    ],
    countries: [
      'Global',
    ],
    features: [
      'EA Support',
      'API Access',
      'Islamic Account',
      'Copy Trading',
      'Islamic Friendly',
      'Trading Signals',
    ],
  },
  {
    id: 'hycm',
    name: 'HYCM',
    logoUrl: '/broker-logos/hycm.png',
    websiteUrl: 'https://www.hycm.com/',
    score: 8.4,
    foundingYear: 1977,
    headquarters: 'London, UK',
    description: 'HYCM (Henyep Capital Markets) is a well-established multi-regulated broker with a long history in the financial markets, offering a reliable trading experience on MT4 and MT5.',
    rating: 9,
    regulation: [
      'FCA',
      'CySEC',
      'DFSA',
      'CIMA',
    ],
    minDeposit: 100,
    spreads: {
      eurusd: 1.2,
      gbpusd: 1.5,
      usdjpy: 1.4,
    },
    accessibility: {
      minDeposit: 100,
      depositMethods: [
        'Credit Card',
        'Skrill',
      ],
      withdrawalMethods: [
        'Credit Card',
        'Skrill',
      ],
      customerSupport: [
        '24/5 Live Chat',
        'Phone',
      ],
    },
    brokerType: 'Market Maker',
    platforms: [
      'MT4',
      'MT5',
    ],
    countries: [
      'Global',
      'Global',
      'Global',
      'Global',
    ],
    features: [
      'EA Support',
      'Islamic Account',
      'Islamic Friendly',
      'Trading Signals',
    ],
  },
  {
    id: 'city-index',
    name: 'City Index',
    logoUrl: '/broker-logos/city-index.png',
    websiteUrl: 'https://www.cityindex.com/',
    score: 8.8,
    foundingYear: 1983,
    headquarters: 'London, UK',
    description: 'City Index is a global CFD and spread betting provider with a long history, strong regulation, and a comprehensive range of markets, owned by StoneX Group.',
    rating: 9.8,
    regulation: [
      'FCA',
      'ASIC',
      'MAS (Singapore)',
    ],
    minDeposit: 100,
    spreads: {
      eurusd: 0.8,
      gbpusd: 1.2,
      usdjpy: 1,
    },
    accessibility: {
      minDeposit: 100,
      depositMethods: [
        'Credit Card',
        'Bank Transfer',
        'PayPal',
      ],
      withdrawalMethods: [
        'Credit Card',
        'Bank Transfer',
        'PayPal',
      ],
      customerSupport: [
        '24/5 Live Chat',
        'Phone',
      ],
    },
    brokerType: 'Market Maker',
    platforms: [
      'Web Trader',
      'AT Pro',
      'MT4',
      'TradingView',
    ],
    countries: [
      'Global',
      'Global',
      'Global',
    ],
    features: [
      'EA Support',
      'API Access',
      'Trading Signals',
    ],
  },
  {
    id: 'activtrades',
    name: 'ActivTrades',
    logoUrl: '/broker-logos/activtrades.png',
    websiteUrl: 'https://www.activtrades.com/',
    score: 8.5,
    foundingYear: 2001,
    headquarters: 'London, UK',
    description: 'ActivTrades is an FCA-regulated broker offering its own advanced ActivTrader platform, known for its additional insurance coverage and enhanced trading tools.',
    rating: 9.2,
    regulation: [
      'FCA',
      'SCB',
      'CSSF',
    ],
    minDeposit: 500,
    spreads: {
      eurusd: 0.8,
      gbpusd: 1.2,
      usdjpy: 1,
    },
    accessibility: {
      minDeposit: 500,
      depositMethods: [
        'Credit Card',
        'Skrill',
        'PayPal',
      ],
      withdrawalMethods: [
        'Credit Card',
        'Skrill',
        'PayPal',
      ],
      customerSupport: [
        '24/5 Live Chat',
        'Phone',
      ],
    },
    brokerType: 'STP',
    platforms: [
      'ActivTrader',
      'MT4',
      'MT5',
    ],
    countries: [
      'Global',
      'Global',
      'Global',
    ],
    features: [
      'EA Support',
      'Islamic Account',
      'Islamic Friendly',
      'Trading Signals',
    ],
  },
  {
    id: 'atfx',
    name: 'ATFX',
    logoUrl: '/broker-logos/ATFX.png',
    websiteUrl: 'https://www.atfx.com/',
    score: 8.3,
    foundingYear: 2017,
    headquarters: 'London, UK',
    description: 'ATFX is a global online CFD broker with a strong regulatory framework, focusing on providing a straightforward MT4 trading experience with competitive pricing.',
    rating: 8.8,
    regulation: [
      'FCA',
      'CySEC',
      'FSC',
    ],
    minDeposit: 100,
    spreads: {
      eurusd: 1.2,
      gbpusd: 1.5,
      usdjpy: 1.3,
    },
    accessibility: {
      minDeposit: 100,
      depositMethods: [
        'Credit Card',
        'Skrill',
      ],
      withdrawalMethods: [
        'Credit Card',
        'Skrill',
      ],
      customerSupport: [
        '24/5 Live Chat',
        'Phone',
      ],
    },
    brokerType: 'STP',
    platforms: [
      'MT4',
    ],
    countries: [
      'Global',
      'Global',
      'Global',
    ],
    features: [
      'EA Support',
      'Islamic Account',
      'Islamic Friendly',
      'Trading Signals',
    ],
  },
  {
    id: 'lcg',
    name: 'LCG',
    logoUrl: '/broker-logos/lcg.jpg',
    websiteUrl: 'https://www.lcg.com/',
    score: 8.2,
    foundingYear: 1996,
    headquarters: 'London, UK',
    description: 'London Capital Group (LCG) is a well-established, FCA-regulated broker offering spread betting and CFD trading on its proprietary LCG Trader platform and MT4.',
    rating: 9,
    regulation: [
      'FCA',
      'CySEC',
    ],
    minDeposit: 0,
    spreads: {
      eurusd: 1.45,
      gbpusd: 1.8,
      usdjpy: 1.5,
    },
    accessibility: {
      minDeposit: 0,
      depositMethods: [
        'Credit Card',
        'Skrill',
      ],
      withdrawalMethods: [
        'Credit Card',
        'Skrill',
      ],
      customerSupport: [
        '24/5 Live Chat',
        'Phone',
      ],
    },
    brokerType: 'Market Maker',
    platforms: [
      'LCG Trader',
      'MT4',
    ],
    countries: [
      'Global',
      'Global',
    ],
    features: [
      'EA Support',
      'Trading Signals',
    ],
  },
  {
    id: 'markets-com',
    name: 'Markets.com',
    logoUrl: '/broker-logos/marketscom.png',
    websiteUrl: 'https://www.markets.com/',
    score: 8.6,
    foundingYear: 2008,
    headquarters: 'Road Town, British Virgin Islands',
    description: 'Markets.com is a well-regulated global broker, part of the Finalto group, offering a comprehensive proprietary trading platform with excellent analytical tools.',
    rating: 9.2,
    regulation: [
      'FCA',
      'CySEC',
      'ASIC',
    ],
    minDeposit: 100,
    spreads: {
      eurusd: 1.7,
      gbpusd: 2.2,
      usdjpy: 1.9,
    },
    accessibility: {
      minDeposit: 100,
      depositMethods: [
        'Credit Card',
        'Skrill',
        'PayPal',
      ],
      withdrawalMethods: [
        'Credit Card',
        'Skrill',
        'PayPal',
      ],
      customerSupport: [
        '24/5 Live Chat',
        'Phone',
      ],
    },
    brokerType: 'Market Maker',
    platforms: [
      'Markets.com Platform',
      'MT4',
      'MT5',
    ],
    countries: [
      'Global',
      'Global',
      'Global',
    ],
    features: [
      'EA Support',
      'Islamic Account',
      'Islamic Friendly',
      'Trading Signals',
    ],
  },
  {
    id: 'ironfx',
    name: 'IronFX',
    logoUrl: '/broker-logos/ironfx.png',
    websiteUrl: 'https://www.ironfx.com/',
    score: 7.8,
    foundingYear: 2010,
    headquarters: 'Limassol, Cyprus',
    description: 'IronFX is a global broker with a wide reach, known for its extensive range of account types, frequent trading competitions, and high leverage options.',
    rating: 7,
    regulation: [
      'CySEC',
    ],
    minDeposit: 100,
    spreads: {
      eurusd: 1.2,
      gbpusd: 1.5,
      usdjpy: 1.4,
    },
    accessibility: {
      minDeposit: 100,
      depositMethods: [
        'Credit Card',
        'Skrill',
      ],
      withdrawalMethods: [
        'Credit Card',
        'Skrill',
      ],
      customerSupport: [
        '24/5 Live Chat',
        'Phone',
      ],
    },
    brokerType: 'STP',
    platforms: [
      'MT4',
    ],
    countries: [
      'Global',
    ],
    features: [
      'EA Support',
      'Islamic Account',
      'Islamic Friendly',
      'Trading Signals',
    ],
  },
  {
    id: 'gmo-click',
    name: 'GMO Click',
    logoUrl: '/broker-logos/gmoclick.png',
    websiteUrl: 'https://www.gmo-click.com/',
    score: 8,
    foundingYear: 2005,
    headquarters: 'Tokyo, Japan',
    description: "GMO Click is a major Japanese forex broker, known for being one of the world's largest by trading volume, offering extremely tight spreads and a robust trading infrastructure.",
    rating: 9.5,
    regulation: [
      'FSA (Japan)',
    ],
    minDeposit: 0,
    spreads: {
      eurusd: 0.3,
      gbpusd: 0.8,
      usdjpy: 0.2,
    },
    accessibility: {
      minDeposit: 0,
      depositMethods: [
        'Bank Transfer',
      ],
      withdrawalMethods: [
        'Bank Transfer',
      ],
      customerSupport: [
        'Phone',
        'Email',
      ],
    },
    brokerType: 'Market Maker',
    platforms: [
      'Platinum Chart',
      'FXroid Plus',
    ],
    countries: [
      'Global',
    ],
    features: [
      'API Access',
      'Trading Signals',
    ],
  },
  {
    id: 'gkfx',
    name: 'GKFX',
    logoUrl: '/broker-logos/gkfx.png',
    websiteUrl: 'https://www.gkfx.com/',
    score: 8,
    foundingYear: 2010,
    headquarters: 'London, UK',
    description: 'GKFX is a global CFD broker offering a wide range of markets on the MetaTrader platforms, backed by strong regulation in several jurisdictions.',
    rating: 8.9,
    regulation: [
      'FCA',
      'BaFin',
    ],
    minDeposit: 0,
    spreads: {
      eurusd: 1.5,
      gbpusd: 1.8,
      usdjpy: 1.6,
    },
    accessibility: {
      minDeposit: 0,
      depositMethods: [
        'Credit Card',
        'Skrill',
      ],
      withdrawalMethods: [
        'Credit Card',
        'Skrill',
      ],
      customerSupport: [
        '24/5 Live Chat',
        'Phone',
      ],
    },
    brokerType: 'STP',
    platforms: [
      'MT4',
      'MT5',
    ],
    countries: [
      'Global',
      'Global',
    ],
    features: [
      'EA Support',
      'Islamic Account',
      'Islamic Friendly',
      'Trading Signals',
    ],
  },
  {
    id: 'bitget',
    name: 'Bitget (CFD)',
    logoUrl: '/broker-logos/bitget.png',
    websiteUrl: 'https://www.bitget.com/',
    score: 8.1,
    foundingYear: 2018,
    headquarters: 'Victoria, Seychelles',
    description: 'Bitget is a leading cryptocurrency exchange that also offers CFD trading on various assets, known for its advanced copy trading features and large user base.',
    rating: 6,
    regulation: [
      'Offshore (Seychelles)',
    ],
    minDeposit: 50,
    spreads: {
      eurusd: 0.8,
      gbpusd: 1.2,
      usdjpy: 1,
    },
    accessibility: {
      minDeposit: 50,
      depositMethods: [
        'Crypto',
        'Credit Card',
      ],
      withdrawalMethods: [
        'Crypto',
      ],
      customerSupport: [
        '24/7 Live Chat',
      ],
    },
    brokerType: 'ECN',
    platforms: [
      'Bitget Platform',
    ],
    countries: [
      'Global',
    ],
    features: [
      'API Access',
      'Copy Trading',
    ],
  },
  {
    id: 'tradeview',
    name: 'Tradeview',
    logoUrl: '/broker-logos/tradeview.png',
    websiteUrl: 'https://www.tradeviewforex.com/',
    score: 8.6,
    foundingYear: 2004,
    headquarters: 'Grand Cayman, Cayman Islands',
    description: 'Tradeview Markets is a multi-asset online brokerage that provides direct market access (DMA) and ECN liquidity, catering to experienced traders and institutions.',
    rating: 7.5,
    regulation: [
      'CIMA',
    ],
    minDeposit: 100,
    spreads: {
      eurusd: 0.2,
      gbpusd: 0.4,
      usdjpy: 0.3,
    },
    accessibility: {
      minDeposit: 100,
      depositMethods: [
        'Credit Card',
        'Bank Transfer',
        'Skrill',
      ],
      withdrawalMethods: [
        'Credit Card',
        'Bank Transfer',
      ],
      customerSupport: [
        '24/5 Live Chat',
        'Phone',
      ],
    },
    brokerType: 'ECN',
    platforms: [
      'MT4',
      'MT5',
      'cTrader',
      'Currenex',
    ],
    countries: [
      'Global',
    ],
    features: [
      'EA Support',
      'API Access',
      'Islamic Account',
      'Copy Trading',
      'Islamic Friendly',
    ],
  },
  {
    id: 'nordfx',
    name: 'NordFX',
    logoUrl: '/broker-logos/nordfx.png',
    websiteUrl: 'https://nordfx.com/',
    score: 8.1,
    foundingYear: 2008,
    headquarters: 'Port Vila, Vanuatu',
    description: 'NordFX is an international broker offering various account types, including zero-spread accounts and social trading, with a focus on emerging markets.',
    rating: 6.8,
    regulation: [
      'VFSC (Vanuatu)',
    ],
    minDeposit: 10,
    spreads: {
      eurusd: 2,
      gbpusd: 2.2,
      usdjpy: 2.1,
    },
    accessibility: {
      minDeposit: 10,
      depositMethods: [
        'Credit Card',
        'Skrill',
        'Crypto',
      ],
      withdrawalMethods: [
        'Credit Card',
        'Skrill',
        'Crypto',
      ],
      customerSupport: [
        '24/5 Live Chat',
        'Phone',
      ],
    },
    brokerType: 'Hybrid',
    platforms: [
      'MT4',
    ],
    countries: [
      'Global',
    ],
    features: [
      'EA Support',
      'Islamic Account',
      'Copy Trading',
      'Islamic Friendly',
    ],
  },
  {
    id: 'fxopen',
    name: 'FXOpen',
    logoUrl: '/broker-logos/fxopen.png',
    websiteUrl: 'https://www.fxopen.com/',
    score: 8.4,
    foundingYear: 2005,
    headquarters: 'London, UK',
    description: 'FXOpen is a well-established ECN broker offering access to deep liquidity and competitive pricing on the MetaTrader and TickTrader platforms.',
    rating: 9,
    regulation: [
      'FCA',
      'ASIC',
    ],
    minDeposit: 100,
    spreads: {
      eurusd: 0.2,
      gbpusd: 0.5,
      usdjpy: 0.3,
    },
    accessibility: {
      minDeposit: 100,
      depositMethods: [
        'Credit/Debit Card',
        'Bank Transfer',
        'Skrill',
        'Neteller',
        'Crypto',
      ],
      withdrawalMethods: [
        'Credit/Debit Card',
        'Bank Transfer',
        'Skrill',
        'Neteller',
        'Crypto',
      ],
      customerSupport: [
        'Live Chat',
        'Phone',
        'Email',
      ],
    },
    brokerType: 'ECN',
    platforms: [
      'MT4',
      'MT5',
      'TickTrader',
    ],
    countries: [
      'Global',
      'Global',
    ],
    features: [
      'EA Support',
      'Islamic Account',
      'Copy Trading',
      'Islamic Friendly',
    ],
  },
  {
    id: 'royal',
    name: 'Royal',
    logoUrl: '/broker-logos/royal.png',
    websiteUrl: 'https://www.royal.com/',
    score: 8.5,
    foundingYear: 2018,
    headquarters: 'Kingstown, St. Vincent & Grenadines',
    description: 'Royal is a modern forex and CFD broker offering competitive trading conditions with a focus on technology and customer service.',
    rating: 8.5,
    regulation: [
      'FSA',
      'CySEC',
    ],
    minDeposit: 50,
    spreads: {
      eurusd: 0.6,
      gbpusd: 0.9,
      usdjpy: 0.7,
    },
    accessibility: {
      minDeposit: 50,
      depositMethods: [
        'Credit/Debit Card',
        'Bank Transfer',
        'Skrill',
        'Neteller',
        'WebMoney',
        'Crypto',
      ],
      withdrawalMethods: [
        'Credit/Debit Card',
        'Bank Transfer',
        'Skrill',
        'Neteller',
        'WebMoney',
        'Crypto',
      ],
      customerSupport: [
        'Live Chat',
        'Phone',
        'Email',
        'Ticket System',
      ],
    },
    brokerType: 'STP/ECN',
    platforms: [
      'MT4',
      'MT5',
      'Royal WebTrader',
      'Royal Mobile App',
    ],
    countries: [
      'Global',
      'Global',
    ],
    features: [
      'EA Support',
      'Islamic Friendly',
      'Trading Signals',
    ],
  },
  {
    id: 'captrader',
    name: 'CapTrader',
    logoUrl: '/broker-logos/captrader.png',
    websiteUrl: 'https://www.captrader.com/',
    score: 8.7,
    foundingYear: 2015,
    headquarters: 'Limassol, Cyprus',
    description: 'CapTrader is a European forex and CFD broker known for its competitive pricing, advanced trading technology, and strong regulatory framework.',
    rating: 8.7,
    regulation: [
      'BaFin',
      'CySEC',
    ],
    minDeposit: 100,
    spreads: {
      eurusd: 0,
      gbpusd: 0.2,
      usdjpy: 0.1,
    },
    accessibility: {
      minDeposit: 100,
      depositMethods: [
        'Credit/Debit Card',
        'Bank Transfer',
        'Skrill',
        'Neteller',
        'Trustly',
        'Sofort',
      ],
      withdrawalMethods: [
        'Credit/Debit Card',
        'Bank Transfer',
        'Skrill',
        'Neteller',
        'Trustly',
        'Sofort',
      ],
      customerSupport: [
        'Live Chat',
        'Phone',
        'Email',
      ],
    },
    brokerType: 'ECN',
    platforms: [
      'MT4',
      'MT5',
      'CapTrader WebTrader',
      'CapTrader Mobile App',
    ],
    countries: [
      'Global',
      'Global',
    ],
    features: [
      'EA Support',
      'Islamic Friendly',
      'Trading Signals',
    ],
  },
  {
    id: 'mexem',
    name: 'Mexem',
    logoUrl: '/broker-logos/mexem.png',
    websiteUrl: 'https://www.mexem.com/',
    score: 8.3,
    foundingYear: 2017,
    headquarters: 'Port Vila, Vanuatu',
    description: 'Mexem is an international forex and CFD broker offering access to global markets with competitive trading conditions and multiple account types.',
    rating: 8.3,
    regulation: [
      'VFSC',
      'FSA',
    ],
    minDeposit: 10,
    spreads: {
      eurusd: 0.8,
      gbpusd: 1.2,
      usdjpy: 0.9,
    },
    accessibility: {
      minDeposit: 10,
      depositMethods: [
        'Credit/Debit Card',
        'Bank Transfer',
        'Skrill',
        'Neteller',
        'WebMoney',
        'Perfect Money',
        'Crypto',
      ],
      withdrawalMethods: [
        'Credit/Debit Card',
        'Bank Transfer',
        'Skrill',
        'Neteller',
        'WebMoney',
        'Perfect Money',
        'Crypto',
      ],
      customerSupport: [
        'Live Chat',
        'Phone',
        'Email',
        'Ticket System',
      ],
    },
    brokerType: 'Market Maker/STP',
    platforms: [
      'MT4',
      'MT5',
      'Mexem WebTrader',
      'Mexem Mobile App',
    ],
    countries: [
      'Global',
      'Global',
    ],
    features: [
      'EA Support',
      'Islamic Friendly',
    ],
  },
  {
    id: 'trading212',
    name: 'Trading 212',
    logoUrl: '/broker-logos/trading212.png',
    websiteUrl: 'https://www.trading212.com/',
    score: 9.6,
    foundingYear: 2004,
    headquarters: 'London, UK',
    description: 'Trading 212 UK Ltd is the commission-free revolution pioneer that transformed global investing. Serving 2+ million clients worldwide, Trading 212 offers zero-commission stocks, ETFs, and CFDs through ...',
    rating: 9.6,
    regulation: [
      'FCA',
      'FSC',
    ],
    minDeposit: 1,
    spreads: {
      eurusd: 0.6,
      gbpusd: 0.8,
      usdjpy: 0.7,
    },
    accessibility: {
      minDeposit: 1,
      depositMethods: [
        'Credit/Debit Card',
        'Bank Transfer',
        'Skrill',
        'Neteller',
        'Trustly',
        'Sofort',
        'Apple Pay',
        'Google Pay',
      ],
      withdrawalMethods: [
        'Credit/Debit Card',
        'Bank Transfer',
        'Skrill',
        'Neteller',
        'Trustly',
        'Sofort',
      ],
      customerSupport: [
        'Email',
        'Ticket System',
        'Help Center',
      ],
    },
    brokerType: 'Market Maker',
    platforms: [
      'Trading 212 Web Platform',
      'Trading 212 Mobile App',
    ],
    countries: [
      'Global',
      'Global',
    ],
    features: [],
  },
  {
    id: 'vt-markets',
    name: 'VT Markets',
    logoUrl: '/broker-logos/vt-markets.png',
    websiteUrl: 'https://www.vtmarkets.com/',
    score: 8.4,
    foundingYear: 2015,
    headquarters: 'Sydney, Australia',
    description: 'VT Markets is an Australian forex and CFD broker offering competitive trading conditions with ASIC regulation and multiple account types.',
    rating: 8.4,
    regulation: [
      'ASIC',
      'VFSC',
    ],
    minDeposit: 100,
    spreads: {
      eurusd: 0,
      gbpusd: 0.5,
      usdjpy: 0.3,
    },
    accessibility: {
      minDeposit: 100,
      depositMethods: [
        'Credit/Debit Card',
        'Bank Transfer',
        'Skrill',
        'Neteller',
        'FasaPay',
        'Crypto',
      ],
      withdrawalMethods: [
        'Credit/Debit Card',
        'Bank Transfer',
        'Skrill',
        'Neteller',
        'FasaPay',
        'Crypto',
      ],
      customerSupport: [
        'Live Chat',
        'Phone',
        'Email',
        'WhatsApp',
      ],
    },
    brokerType: 'ECN/STP',
    platforms: [
      'MT4',
      'MT5',
      'VT Markets Mobile App',
    ],
    countries: [
      'Global',
      'Global',
    ],
    features: [
      'EA Support',
      'Islamic Friendly',
    ],
  },
  {
    id: 'tmgm',
    name: 'TMGM',
    logoUrl: '/broker-logos/tmgm.png',
    websiteUrl: 'https://www.tmgm.com/',
    score: 8.6,
    foundingYear: 2013,
    headquarters: 'Auckland, New Zealand',
    description: 'TMGM (Trademax Global Markets) is a New Zealand-based forex and CFD broker offering competitive trading conditions with multiple regulatory licenses.',
    rating: 8.6,
    regulation: [
      'FMA',
      'ASIC',
      'VFSC',
    ],
    minDeposit: 100,
    spreads: {
      eurusd: 0,
      gbpusd: 0.4,
      usdjpy: 0.3,
    },
    accessibility: {
      minDeposit: 100,
      depositMethods: [
        'Credit/Debit Card',
        'Bank Transfer',
        'Skrill',
        'Neteller',
        'SticPay',
        'Crypto',
      ],
      withdrawalMethods: [
        'Credit/Debit Card',
        'Bank Transfer',
        'Skrill',
        'Neteller',
        'SticPay',
        'Crypto',
      ],
      customerSupport: [
        'Live Chat',
        'Phone',
        'Email',
        'WhatsApp',
      ],
    },
    brokerType: 'ECN',
    platforms: [
      'MT4',
      'MT5',
      'TMGM Trader',
      'TMGM Mobile App',
    ],
    countries: [
      'Global',
      'Global',
      'Global',
    ],
    features: [
      'EA Support',
      'Islamic Friendly',
      'Trading Signals',
    ],
  },
  {
    id: 'trade-nation',
    name: 'Trade Nation',
    logoUrl: '/broker-logos/trade-nation.png',
    websiteUrl: 'https://www.tradenation.com/',
    score: 8.8,
    foundingYear: 2014,
    headquarters: 'London, UK',
    description: 'Trade Nation is a UK-based forex and CFD broker known for its transparent pricing, excellent customer service, and strong regulatory framework.',
    rating: 8.8,
    regulation: [
      'FCA',
      'ASIC',
      'FSCA',
    ],
    minDeposit: 1,
    spreads: {
      eurusd: 0.6,
      gbpusd: 0.9,
      usdjpy: 0.7,
    },
    accessibility: {
      minDeposit: 1,
      depositMethods: [
        'Credit/Debit Card',
        'Bank Transfer',
        'Skrill',
        'Neteller',
        'Trustly',
        'Sofort',
      ],
      withdrawalMethods: [
        'Credit/Debit Card',
        'Bank Transfer',
        'Skrill',
        'Neteller',
        'Trustly',
        'Sofort',
      ],
      customerSupport: [
        'Live Chat',
        'Phone',
        'Email',
      ],
    },
    brokerType: 'Market Maker',
    platforms: [
      'MT4',
      'MT5',
      'Trade Nation Mobile App',
    ],
    countries: [
      'Global',
      'Global',
      'Global',
    ],
    features: [
      'Islamic Friendly',
      'Trading Signals',
    ],
  },
  {
    id: 'fx-trading',
    name: 'FX Trading.com',
    logoUrl: '/broker-logos/fx-trading.png',
    websiteUrl: 'https://www.fxtrading.com/',
    score: 8.2,
    foundingYear: 2016,
    headquarters: 'Port Vila, Vanuatu',
    description: 'FX Trading.com is an international forex and CFD broker offering competitive trading conditions with multiple account types and regulatory oversight.',
    rating: 8.2,
    regulation: [
      'VFSC',
      'FSA',
    ],
    minDeposit: 100,
    spreads: {
      eurusd: 0.8,
      gbpusd: 1.2,
      usdjpy: 0.9,
    },
    accessibility: {
      minDeposit: 100,
      depositMethods: [
        'Credit/Debit Card',
        'Bank Transfer',
        'Skrill',
        'Neteller',
        'WebMoney',
        'Perfect Money',
        'Crypto',
      ],
      withdrawalMethods: [
        'Credit/Debit Card',
        'Bank Transfer',
        'Skrill',
        'Neteller',
        'WebMoney',
        'Perfect Money',
        'Crypto',
      ],
      customerSupport: [
        'Live Chat',
        'Phone',
        'Email',
        'Ticket System',
      ],
    },
    brokerType: 'ECN/STP',
    platforms: [
      'MT4',
      'MT5',
      'FX Trading WebTrader',
      'FX Trading Mobile App',
    ],
    countries: [
      'Global',
      'Global',
    ],
    features: [
      'EA Support',
      'Islamic Friendly',
      'Trading Signals',
    ],
  },
  {
    id: 'multibank',
    name: 'Multibank',
    logoUrl: '/broker-logos/multibank.png',
    websiteUrl: 'https://www.multibank.com/',
    score: 8.5,
    foundingYear: 2005,
    headquarters: 'Limassol, Cyprus',
    description: 'Multibank is a global forex and CFD broker with multiple regulatory licenses and a strong presence in international markets.',
    rating: 8.5,
    regulation: [
      'CySEC',
      'FCA',
      'ASIC',
      'FSB',
    ],
    minDeposit: 50,
    spreads: {
      eurusd: 0.2,
      gbpusd: 0.4,
      usdjpy: 0.3,
    },
    accessibility: {
      minDeposit: 50,
      depositMethods: [
        'Credit/Debit Card',
        'Bank Transfer',
        'Skrill',
        'Neteller',
        'WebMoney',
        'Crypto',
      ],
      withdrawalMethods: [
        'Credit/Debit Card',
        'Bank Transfer',
        'Skrill',
        'Neteller',
        'WebMoney',
        'Crypto',
      ],
      customerSupport: [
        'Live Chat',
        'Phone',
        'Email',
        'WhatsApp',
      ],
    },
    brokerType: 'ECN/STP',
    platforms: [
      'MT4',
      'MT5',
      'Multibank Trader',
      'Multibank Mobile App',
    ],
    countries: [
      'Global',
      'Global',
      'Global',
      'Global',
    ],
    features: [
      'EA Support',
      'Islamic Friendly',
      'Trading Signals',
    ],
  },
  {
    id: 'tradestation-global',
    name: 'Tradestation Global',
    logoUrl: '/broker-logos/tradestation-global.png',
    websiteUrl: 'https://www.tradestationglobal.com/',
    score: 8.9,
    foundingYear: 1982,
    headquarters: 'Plantation, Florida, USA',
    description: 'Tradestation Global is a US-based broker known for its advanced trading platform, powerful tools, and comprehensive market access.',
    rating: 8.9,
    regulation: [
      'SEC',
      'FINRA',
      'SIPC',
      'FCA',
    ],
    minDeposit: 500,
    spreads: {
      eurusd: 0.8,
      gbpusd: 1.2,
      usdjpy: 0.9,
    },
    accessibility: {
      minDeposit: 500,
      depositMethods: [
        'Credit/Debit Card',
        'Bank Transfer',
        'ACH Transfer',
        'Wire Transfer',
      ],
      withdrawalMethods: [
        'Credit/Debit Card',
        'Bank Transfer',
        'ACH Transfer',
        'Wire Transfer',
      ],
      customerSupport: [
        'Phone',
        'Email',
        'Ticket System',
        'Live Chat',
      ],
    },
    brokerType: 'ECN',
    platforms: [
      'Tradestation Platform',
      'Tradestation Mobile',
      'Matrix',
      'OptionStation',
    ],
    countries: [
      'Global',
      'Global',
      'Global',
      'Global',
    ],
    features: [
      'EA Support',
      'API Access',
    ],
  },
  {
    id: 'spreadex',
    name: 'Spreadex',
    logoUrl: '/broker-logos/spreadex.png',
    websiteUrl: 'https://www.spreadex.com/',
    score: 8.7,
    foundingYear: 1999,
    headquarters: 'Stoke-on-Trent, UK',
    description: 'Spreadex is a UK-based financial spread betting and CFD broker with FCA regulation and a focus on sports betting.',
    rating: 8.7,
    regulation: [
      'FCA',
    ],
    minDeposit: 100,
    spreads: {
      eurusd: 0.8,
      gbpusd: 1.2,
      usdjpy: 0.9,
    },
    accessibility: {
      minDeposit: 100,
      depositMethods: [
        'Credit/Debit Card',
        'Bank Transfer',
      ],
      withdrawalMethods: [
        'Credit/Debit Card',
        'Bank Transfer',
      ],
      customerSupport: [
        'Phone',
        'Email',
        'Live Chat',
      ],
    },
    brokerType: 'Market Maker',
    platforms: [
      'Spreadex Platform',
      'Spreadex Mobile App',
      'MT4',
    ],
    countries: [
      'Global',
    ],
    features: [
      'Trading Signals',
    ],
  },
  {
    id: 'fusion-markets',
    name: 'Fusion Markets',
    logoUrl: '/broker-logos/fusion-markets.png',
    websiteUrl: 'https://www.fusionmarkets.com/',
    score: 8.7,
    foundingYear: 2018,
    headquarters: 'Melbourne, Australia',
    description: 'Fusion Markets is an Australian-based ECN/STP broker known for offering some of the lowest commissions in the industry. The company focuses on providing transparent pricing and direct market access to...',
    rating: 8.7,
    regulation: [
      'ASIC',
      'VFSC',
    ],
    minDeposit: 100,
    spreads: {
      eurusd: 0,
      gbpusd: 0.2,
      usdjpy: 0.1,
    },
    accessibility: {
      minDeposit: 100,
      depositMethods: [
        'Bank Transfer',
        'Credit/Debit Card',
        'Skrill',
        'Neteller',
        'PayPal',
      ],
      withdrawalMethods: [
        'Bank Transfer',
        'Credit/Debit Card',
        'Skrill',
        'Neteller',
        'PayPal',
      ],
      customerSupport: [
        'Phone',
        'Email',
        'Live Chat',
      ],
    },
    brokerType: 'Unknown',
    platforms: [
      'MT4',
      'MT5',
      'cTrader',
    ],
    countries: [
      'Global',
      'Global',
    ],
    features: [
      'EA Support',
      'API Access',
      'Trading Signals',
    ],
  },
  {
    id: 'eightcap',
    name: 'Eightcap',
    logoUrl: '/broker-logos/eightcap.png',
    websiteUrl: 'https://www.eightcap.com/',
    score: 8.4,
    foundingYear: 2009,
    headquarters: 'Melbourne, Australia',
    description: 'Eightcap is an Australian-based multi-asset broker offering forex, indices, commodities, shares, and cryptocurrency trading. The broker is known for competitive pricing and strong regulatory oversight...',
    rating: 8.4,
    regulation: [
      'ASIC',
      'FCA',
    ],
    minDeposit: 100,
    spreads: {
      eurusd: 0,
      gbpusd: 0,
      usdjpy: 0,
    },
    accessibility: {
      minDeposit: 100,
      depositMethods: [
        'Bank Transfer',
        'Credit/Debit Card',
        'Skrill',
        'Neteller',
        'PayPal',
      ],
      withdrawalMethods: [
        'Bank Transfer',
        'Credit/Debit Card',
        'Skrill',
        'Neteller',
        'PayPal',
      ],
      customerSupport: [
        'Phone',
        'Email',
        'Live Chat',
      ],
    },
    brokerType: 'Unknown',
    platforms: [
      'MT4',
      'MT5',
      'TradingView',
    ],
    countries: [
      'Global',
      'Global',
    ],
    features: [
      'EA Support',
      'API Access',
      'Trading Signals',
    ],
  },
  {
    id: 'hantec-markets',
    name: 'Hantec Markets',
    logoUrl: '/broker-logos/hantec-markets.png',
    websiteUrl: 'https://www.hantecmarkets.com/',
    score: 8.3,
    foundingYear: 1990,
    headquarters: 'London, UK',
    description: 'Hantec Markets is a well-established UK-based forex and CFD broker with over 30 years of experience. The company offers a comprehensive trading platform with strong regulatory oversight.',
    rating: 8.3,
    regulation: [
      'FCA',
    ],
    minDeposit: 100,
    spreads: {
      eurusd: 0.7,
      gbpusd: 1,
      usdjpy: 0.8,
    },
    accessibility: {
      minDeposit: 100,
      depositMethods: [
        'Bank Transfer',
        'Credit/Debit Card',
        'Skrill',
        'Neteller',
      ],
      withdrawalMethods: [
        'Bank Transfer',
        'Credit/Debit Card',
        'Skrill',
        'Neteller',
      ],
      customerSupport: [
        'Phone',
        'Email',
        'Live Chat',
      ],
    },
    brokerType: 'Unknown',
    platforms: [
      'MT4',
      'MT5',
    ],
    countries: [
      'Global',
    ],
    features: [
      'EA Support',
      'API Access',
      'Trading Signals',
    ],
  },
  {
    id: 'global-prime',
    name: 'Global Prime',
    logoUrl: '/broker-logos/global-prime.png',
    websiteUrl: 'https://www.globalprime.com/',
    score: 8.6,
    foundingYear: 2010,
    headquarters: 'Sydney, Australia',
    description: 'Global Prime is an Australian-based ECN/STP broker specializing in forex and CFD trading. The company is known for transparent pricing and direct market access with no dealing desk.',
    rating: 8.6,
    regulation: [
      'ASIC',
    ],
    minDeposit: 200,
    spreads: {
      eurusd: 0,
      gbpusd: 0.3,
      usdjpy: 0.2,
    },
    accessibility: {
      minDeposit: 200,
      depositMethods: [
        'Bank Transfer',
        'Credit/Debit Card',
        'Skrill',
        'Neteller',
      ],
      withdrawalMethods: [
        'Bank Transfer',
        'Credit/Debit Card',
        'Skrill',
        'Neteller',
      ],
      customerSupport: [
        'Phone',
        'Email',
        'Live Chat',
      ],
    },
    brokerType: 'Unknown',
    platforms: [
      'MT4',
      'MT5',
    ],
    countries: [
      'Global',
    ],
    features: [
      'EA Support',
      'API Access',
      'Trading Signals',
    ],
  },
  {
    id: 'go-markets',
    name: 'Go Markets',
    logoUrl: '/broker-logos/go-markets.png',
    websiteUrl: 'https://www.gomarkets.com/',
    score: 8.4,
    foundingYear: 2006,
    headquarters: 'Melbourne, Australia',
    description: "Go Markets is one of Australia's first MetaTrader brokers, offering forex and CFD trading services since 2006. The company has built a strong reputation for reliability and customer service.",
    rating: 8.4,
    regulation: [
      'ASIC',
    ],
    minDeposit: 100,
    spreads: {
      eurusd: 0.8,
      gbpusd: 1,
      usdjpy: 0.8,
    },
    accessibility: {
      minDeposit: 100,
      depositMethods: [
        'Bank Transfer',
        'Credit/Debit Card',
        'Skrill',
        'Neteller',
      ],
      withdrawalMethods: [
        'Bank Transfer',
        'Credit/Debit Card',
        'Skrill',
        'Neteller',
      ],
      customerSupport: [
        'Phone',
        'Email',
        'Live Chat',
      ],
    },
    brokerType: 'Unknown',
    platforms: [
      'MT4',
      'MT5',
    ],
    countries: [
      'Global',
    ],
    features: [
      'EA Support',
      'API Access',
      'Trading Signals',
    ],
  },
  {
    id: 'moneta-markets',
    name: 'Moneta Markets',
    logoUrl: '/broker-logos/moneta-markets.png',
    websiteUrl: 'https://www.monetamarkets.com/',
    score: 8.2,
    foundingYear: 2020,
    headquarters: 'Kingstown, St. Vincent & Grenadines',
    description: 'Moneta Markets is an international forex and CFD broker offering a wide range of trading instruments. The broker focuses on providing competitive spreads and a user-friendly trading experience.',
    rating: 8.2,
    regulation: [
      'FSCA',
      'VFSC',
    ],
    minDeposit: 100,
    spreads: {
      eurusd: 0.8,
      gbpusd: 1.2,
      usdjpy: 0.9,
    },
    accessibility: {
      minDeposit: 100,
      depositMethods: [
        'Bank Transfer',
        'Credit/Debit Card',
        'Skrill',
        'Neteller',
        'PayPal',
      ],
      withdrawalMethods: [
        'Bank Transfer',
        'Credit/Debit Card',
        'Skrill',
        'Neteller',
        'PayPal',
      ],
      customerSupport: [
        'Phone',
        'Email',
        'Live Chat',
      ],
    },
    brokerType: 'Unknown',
    platforms: [
      'MT4',
      'MT5',
      'WebTrader',
    ],
    countries: [
      'Global',
      'Global',
    ],
    features: [
      'EA Support',
      'API Access',
      'Trading Signals',
    ],
  },
  {
    id: 'blackbull',
    name: 'BlackBull Markets',
    logoUrl: '/broker-logos/blackbull.png',
    websiteUrl: 'https://www.blackbull.com/',
    score: 8.6,
    foundingYear: 2013,
    headquarters: 'Auckland, New Zealand',
    description: 'BlackBull Markets is a globally regulated forex and CFD broker offering competitive trading conditions with ECN pricing, low commissions, and fast execution speeds.',
    rating: 9,
    regulation: [
      'FMA',
      'FSCA',
      'FSA Seychelles',
    ],
    minDeposit: 100,
    spreads: {
      eurusd: 0,
      gbpusd: 0.1,
      usdjpy: 0.1,
    },
    accessibility: {
      minDeposit: 100,
      depositMethods: [
        'Bank Transfer',
        'Credit/Debit Card',
        'Skrill',
        'Neteller',
        'Crypto',
      ],
      withdrawalMethods: [
        'Bank Transfer',
        'Credit/Debit Card',
        'Skrill',
        'Neteller',
        'Crypto',
      ],
      customerSupport: [
        '24/7 Live Chat',
        'Phone',
        'Email',
      ],
    },
    brokerType: 'ECN',
    platforms: [
      'MT4',
      'MT5',
      'cTrader',
    ],
    countries: [
      'Global',
      'Global',
      'Global',
    ],
    features: [
      'EA Support',
      'API Access',
      'Islamic Account',
      'Copy Trading',
      'Islamic Friendly',
      'Trading Signals',
    ],
  },
  {
    id: 'startrader',
    name: 'StarTrader',
    logoUrl: '/broker-logos/startrader.png',
    websiteUrl: 'https://www.startrader.com/',
    score: 8.4,
    foundingYear: 2010,
    headquarters: 'Limassol, Cyprus',
    description: 'StarTrader is a multi-regulated forex and CFD broker offering competitive trading conditions with STP/ECN execution, low spreads, and a variety of account types.',
    rating: 9.2,
    regulation: [
      'CySEC',
      'FCA',
      'ASIC',
    ],
    minDeposit: 100,
    spreads: {
      eurusd: 1,
      gbpusd: 1.2,
      usdjpy: 1.1,
    },
    accessibility: {
      minDeposit: 100,
      depositMethods: [
        'Bank Transfer',
        'Credit/Debit Card',
        'Skrill',
        'Neteller',
      ],
      withdrawalMethods: [
        'Bank Transfer',
        'Credit/Debit Card',
        'Skrill',
        'Neteller',
      ],
      customerSupport: [
        '24/5 Live Chat',
        'Phone',
        'Email',
      ],
    },
    brokerType: 'ECN/STP',
    platforms: [
      'MT4',
      'MT5',
    ],
    countries: [
      'Global',
      'Global',
      'Global',
    ],
    features: [
      'EA Support',
      'API Access',
      'Islamic Account',
      'Islamic Friendly',
    ],
  },
  {
    id: 'libertex',
    name: 'Libertex',
    logoUrl: '/broker-logos/libertex.png',
    websiteUrl: 'https://libertex.org/',
    score: 8.5,
    foundingYear: 1997,
    headquarters: 'Limassol, Cyprus',
    description: 'Libertex is an award-winning online broker with over 20 years of experience, offering commission-based trading with zero spreads and a user-friendly platform.',
    rating: 8.5,
    regulation: [
      'CySEC',
      'FSCA',
    ],
    minDeposit: 10,
    spreads: {
      eurusd: 0,
      gbpusd: 0,
      usdjpy: 0,
    },
    accessibility: {
      minDeposit: 10,
      depositMethods: [
        'Bank Transfer',
        'Credit/Debit Card',
        'Skrill',
        'Neteller',
        'WebMoney',
      ],
      withdrawalMethods: [
        'Bank Transfer',
        'Credit/Debit Card',
        'Skrill',
        'Neteller',
        'WebMoney',
      ],
      customerSupport: [
        '24/5 Live Chat',
        'Phone',
        'Email',
      ],
    },
    brokerType: 'Market Maker',
    platforms: [
      'Libertex Platform',
      'MT4',
      'MT5',
      'WebTrader',
    ],
    countries: [
      'Global',
      'Global',
    ],
    features: [
      'EA Support',
      'Islamic Account',
      'Islamic Friendly',
    ],
  },
  {
    id: 'capital-com',
    name: 'Capital.com',
    logoUrl: '/broker-logos/capital-com.png',
    websiteUrl: 'https://capital.com/',
    score: 9.5,
    foundingYear: 2016,
    headquarters: 'London, UK',
    description: 'Capital.com Ltd is the AI-powered research authority revolutionizing trader education since 2016. With explosive growth across multiple tier-1 jurisdictions (FCA, ASIC, CySEC), Capital.com combines cu...',
    rating: 9.5,
    regulation: [
      'FCA',
      'ASIC',
      'CySEC',
    ],
    minDeposit: 0,
    spreads: {
      eurusd: 0.67,
      gbpusd: 0.9,
      usdjpy: 0.8,
    },
    accessibility: {
      minDeposit: 0,
      depositMethods: [
        'Bank Transfer',
        'Credit/Debit Card',
        'Skrill',
        'Neteller',
        'PayPal',
      ],
      withdrawalMethods: [
        'Bank Transfer',
        'Credit/Debit Card',
        'Skrill',
        'Neteller',
        'PayPal',
      ],
      customerSupport: [
        '24/5 Live Chat',
        'Phone',
        'Email',
      ],
    },
    brokerType: 'STP',
    platforms: [
      'Capital.com Platform',
      'MT4',
    ],
    countries: [
      'Global',
      'Global',
      'Global',
    ],
    features: [
      'EA Support',
      'API Access',
      'Islamic Account',
      'Islamic Friendly',
    ],
  },
  {
    id: 'blackbull-markets',
    name: 'BlackBull Markets',
    logoUrl: '/broker-logos/blackbull.png',
    websiteUrl: 'https://blackbull.com/',
    score: 8.7,
    foundingYear: 2014,
    headquarters: 'Auckland, New Zealand',
    description: 'BlackBull Markets is a New Zealand-based ECN broker offering raw spreads, fast execution, and access to over 26,000 trading instruments.',
    rating: 7.5,
    regulation: [
      'FMA',
      'FSA',
    ],
    minDeposit: 0,
    spreads: {
      eurusd: 0.11,
      gbpusd: 0.15,
      usdjpy: 0.12,
    },
    accessibility: {
      minDeposit: 0,
      depositMethods: [
        'Bank Transfer',
        'Credit/Debit Card',
        'Skrill',
        'Neteller',
        'FasaPay',
      ],
      withdrawalMethods: [
        'Bank Transfer',
        'Credit/Debit Card',
        'Skrill',
        'Neteller',
        'FasaPay',
      ],
      customerSupport: [
        '24/5 Live Chat',
        'Phone',
        'Email',
      ],
    },
    brokerType: 'ECN',
    platforms: [
      'MT4',
      'MT5',
      'cTrader',
      'TradingView',
    ],
    countries: [
      'Global',
      'Global',
    ],
    features: [
      'EA Support',
      'API Access',
      'Islamic Account',
      'Islamic Friendly',
    ],
  },
  {
    id: 'ictrading',
    name: 'IC Trading',
    logoUrl: '/broker-logos/ictrading.png',
    websiteUrl: 'https://ictrading.com/',
    score: 7.8,
    foundingYear: 2020,
    headquarters: 'Port Louis, Mauritius',
    description: 'IC Trading is a Mauritius-based broker offering competitive spreads from 0.0 pips, high leverage up to 1:500, and access to multiple trading platforms.',
    rating: 6,
    regulation: [
      'FSC',
    ],
    minDeposit: 200,
    spreads: {
      eurusd: 0,
      gbpusd: 0.1,
      usdjpy: 0.1,
    },
    accessibility: {
      minDeposit: 200,
      depositMethods: [
        'Bank Transfer',
        'Credit/Debit Card',
        'Skrill',
        'Neteller',
        'WebMoney',
      ],
      withdrawalMethods: [
        'Bank Transfer',
        'Credit/Debit Card',
        'Skrill',
        'Neteller',
        'WebMoney',
      ],
      customerSupport: [
        '24/7 Live Chat',
        'Phone',
        'Email',
      ],
    },
    brokerType: 'ECN',
    platforms: [
      'MT4',
      'MT5',
      'cTrader',
      'ZuluTrade',
    ],
    countries: [
      'Global',
    ],
    features: [
      'EA Support',
      'API Access',
      'Islamic Account',
      'Copy Trading',
      'Islamic Friendly',
    ],
  },
  {
    id: 'easymarkets',
    name: 'EasyMarkets',
    logoUrl: '/broker-logos/easymarkets.png',
    websiteUrl: 'https://easymarkets.com/',
    score: 8,
    foundingYear: 2001,
    headquarters: 'Limassol, Cyprus',
    description: 'EasyMarkets is a veteran broker offering fixed spreads, guaranteed stop loss, and a user-friendly platform with no hidden fees.',
    rating: 8,
    regulation: [
      'CySEC',
      'ASIC',
    ],
    minDeposit: 100,
    spreads: {
      eurusd: 0.7,
      gbpusd: 1.3,
      usdjpy: 1,
    },
    accessibility: {
      minDeposit: 100,
      depositMethods: [
        'Bank Transfer',
        'Credit/Debit Card',
        'Skrill',
        'Neteller',
        'WebMoney',
      ],
      withdrawalMethods: [
        'Bank Transfer',
        'Credit/Debit Card',
        'Skrill',
        'Neteller',
        'WebMoney',
      ],
      customerSupport: [
        '24/5 Live Chat',
        'Phone',
        'Email',
      ],
    },
    brokerType: 'Market Maker',
    platforms: [
      'EasyMarkets Platform',
      'TradingView',
      'MT4',
      'MT5',
    ],
    countries: [
      'Global',
      'Global',
    ],
    features: [
      'EA Support',
      'Islamic Account',
      'Islamic Friendly',
    ],
  },
  {
    id: 'freedom24',
    name: 'Freedom24',
    logoUrl: '/broker-logos/freedom24.png',
    websiteUrl: 'https://freedom24.com/',
    score: 8.2,
    foundingYear: 2001,
    headquarters: 'Limassol, Cyprus',
    description: 'Freedom24 is a European broker offering access to global markets with competitive pricing and modern trading technology.',
    rating: 7.5,
    regulation: [
      'CySEC',
    ],
    minDeposit: 0,
    spreads: {
      eurusd: 'Competitive',
      gbpusd: 'Competitive',
      usdjpy: 'Competitive',
    },
    accessibility: {
      minDeposit: 0,
      depositMethods: [
        'Bank Transfer',
        'Credit/Debit Card',
      ],
      withdrawalMethods: [
        'Bank Transfer',
        'Credit/Debit Card',
      ],
      customerSupport: [
        '24/5 Live Chat',
        'Phone',
        'Email',
      ],
    },
    brokerType: 'STP',
    platforms: [
      'Freedom24 Platform',
    ],
    countries: [
      'Global',
    ],
    features: [
      'EA Support',
      'API Access',
    ],
  },
  {
    id: 'fxoro',
    name: 'FXORO',
    logoUrl: '/broker-logos/fxoro.png',
    websiteUrl: 'https://fxoro.com/',
    score: 7.5,
    foundingYear: 2012,
    headquarters: 'Limassol, Cyprus',
    description: 'FXORO is a Cyprus-based forex and CFD broker offering multiple trading platforms and competitive trading conditions.',
    rating: 7,
    regulation: [
      'CySEC',
    ],
    minDeposit: 250,
    spreads: {
      eurusd: 1,
      gbpusd: 1.2,
      usdjpy: 1.1,
    },
    accessibility: {
      minDeposit: 250,
      depositMethods: [
        'Bank Transfer',
        'Credit/Debit Card',
        'Skrill',
        'Neteller',
      ],
      withdrawalMethods: [
        'Bank Transfer',
        'Credit/Debit Card',
        'Skrill',
        'Neteller',
      ],
      customerSupport: [
        '24/5 Live Chat',
        'Phone',
        'Email',
      ],
    },
    brokerType: 'Market Maker',
    platforms: [
      'MT4',
      'MT5',
      'WebTrader',
    ],
    countries: [
      'Global',
    ],
    features: [
      'EA Support',
      'Islamic Account',
      'Islamic Friendly',
    ],
  },
  {
    id: 'startrader-global',
    name: 'StarTrader',
    logoUrl: '/broker-logos/startrader.png',
    websiteUrl: 'https://startrader.com/',
    score: 7.8,
    foundingYear: 2019,
    headquarters: 'Limassol, Cyprus',
    description: 'StarTrader is a modern forex and CFD broker offering competitive trading conditions and multiple platform options.',
    rating: 7,
    regulation: [
      'CySEC',
    ],
    minDeposit: 100,
    spreads: {
      eurusd: 1,
      gbpusd: 1.2,
      usdjpy: 1.1,
    },
    accessibility: {
      minDeposit: 100,
      depositMethods: [
        'Bank Transfer',
        'Credit/Debit Card',
        'Skrill',
        'Neteller',
      ],
      withdrawalMethods: [
        'Bank Transfer',
        'Credit/Debit Card',
        'Skrill',
        'Neteller',
      ],
      customerSupport: [
        '24/5 Live Chat',
        'Phone',
        'Email',
      ],
    },
    brokerType: 'STP',
    platforms: [
      'MT4',
      'MT5',
      'WebTrader',
    ],
    countries: [
      'Global',
    ],
    features: [
      'EA Support',
      'Islamic Account',
      'Islamic Friendly',
    ],
  },
  {
    id: 'puprime',
    name: 'PU Prime',
    logoUrl: '/broker-logos/puprime.png',
    websiteUrl: 'https://puprime.com/',
    score: 8,
    foundingYear: 2015,
    headquarters: 'Port Vila, Vanuatu',
    description: 'PU Prime is a global forex and CFD broker offering competitive trading conditions and multiple platform options.',
    rating: 6,
    regulation: [
      'VFSC',
    ],
    minDeposit: 50,
    spreads: {
      eurusd: 1.2,
      gbpusd: 1.4,
      usdjpy: 1.3,
    },
    accessibility: {
      minDeposit: 50,
      depositMethods: [
        'Bank Transfer',
        'Credit/Debit Card',
        'Skrill',
        'Neteller',
        'WebMoney',
      ],
      withdrawalMethods: [
        'Bank Transfer',
        'Credit/Debit Card',
        'Skrill',
        'Neteller',
        'WebMoney',
      ],
      customerSupport: [
        '24/5 Live Chat',
        'Phone',
        'Email',
      ],
    },
    brokerType: 'ECN',
    platforms: [
      'MT4',
      'MT5',
      'WebTrader',
    ],
    countries: [
      'Global',
    ],
    features: [
      'EA Support',
      'API Access',
      'Islamic Account',
      'Islamic Friendly',
    ],
  },
  {
    id: 'gbe-brokers',
    name: 'GBE Brokers',
    logoUrl: '/broker-logos/gbe-brokers.png',
    websiteUrl: 'https://gbebrokers.com/',
    score: 8.1,
    foundingYear: 2012,
    headquarters: 'Limassol, Cyprus',
    description: 'GBE Brokers is a Cyprus-based forex and CFD broker offering competitive trading conditions and regulatory oversight.',
    rating: 7.5,
    regulation: [
      'CySEC',
    ],
    minDeposit: 200,
    spreads: {
      eurusd: 1,
      gbpusd: 1.2,
      usdjpy: 1.1,
    },
    accessibility: {
      minDeposit: 200,
      depositMethods: [
        'Bank Transfer',
        'Credit/Debit Card',
        'Skrill',
        'Neteller',
      ],
      withdrawalMethods: [
        'Bank Transfer',
        'Credit/Debit Card',
        'Skrill',
        'Neteller',
      ],
      customerSupport: [
        '24/5 Live Chat',
        'Phone',
        'Email',
      ],
    },
    brokerType: 'STP',
    platforms: [
      'MT4',
      'MT5',
      'WebTrader',
    ],
    countries: [
      'Global',
    ],
    features: [
      'EA Support',
      'Islamic Account',
      'Islamic Friendly',
    ],
  },
  {
    id: 'thunder-markets',
    name: 'Thunder Markets',
    logoUrl: '/broker-logos/thunder-markets.png',
    websiteUrl: 'https://thundermarkets.com/',
    score: 7.6,
    foundingYear: 2018,
    headquarters: 'Port Vila, Vanuatu',
    description: 'Thunder Markets is a Vanuatu-based forex and CFD broker offering competitive trading conditions and high leverage.',
    rating: 6,
    regulation: [
      'VFSC',
    ],
    minDeposit: 100,
    spreads: {
      eurusd: 1.2,
      gbpusd: 1.4,
      usdjpy: 1.3,
    },
    accessibility: {
      minDeposit: 100,
      depositMethods: [
        'Bank Transfer',
        'Credit/Debit Card',
        'Skrill',
        'Neteller',
      ],
      withdrawalMethods: [
        'Bank Transfer',
        'Credit/Debit Card',
        'Skrill',
        'Neteller',
      ],
      customerSupport: [
        '24/5 Live Chat',
        'Phone',
        'Email',
      ],
    },
    brokerType: 'ECN',
    platforms: [
      'MT4',
      'MT5',
      'WebTrader',
    ],
    countries: [
      'Global',
    ],
    features: [
      'EA Support',
      'API Access',
      'Islamic Account',
      'Islamic Friendly',
    ],
  },
  {
    id: 'superforex',
    name: 'SuperForex',
    logoUrl: '/broker-logos/superforex.png',
    websiteUrl: 'https://superforex.com/',
    score: 7.8,
    foundingYear: 2013,
    headquarters: 'Port Vila, Vanuatu',
    description: 'SuperForex is a global forex and CFD broker offering competitive trading conditions and multiple account types.',
    rating: 6,
    regulation: [
      'VFSC',
    ],
    minDeposit: 1,
    spreads: {
      eurusd: 1.2,
      gbpusd: 1.4,
      usdjpy: 1.3,
    },
    accessibility: {
      minDeposit: 1,
      depositMethods: [
        'Bank Transfer',
        'Credit/Debit Card',
        'Skrill',
        'Neteller',
        'WebMoney',
        'Bitcoin',
      ],
      withdrawalMethods: [
        'Bank Transfer',
        'Credit/Debit Card',
        'Skrill',
        'Neteller',
        'WebMoney',
        'Bitcoin',
      ],
      customerSupport: [
        '24/7 Live Chat',
        'Phone',
        'Email',
      ],
    },
    brokerType: 'ECN',
    platforms: [
      'MT4',
      'MT5',
      'WebTrader',
    ],
    countries: [
      'Global',
    ],
    features: [
      'EA Support',
      'API Access',
      'Islamic Account',
      'Islamic Friendly',
    ],
  },
  {
    id: 'instaforex',
    name: 'InstaForex',
    logoUrl: '/broker-logos/instaforex.png',
    websiteUrl: 'https://instaforex.com/',
    score: 8,
    foundingYear: 2007,
    headquarters: 'Limassol, Cyprus',
    description: 'InstaForex is a well-established forex broker offering competitive trading conditions and innovative trading tools.',
    rating: 7,
    regulation: [
      'CySEC',
      'BVI FSC',
    ],
    minDeposit: 1,
    spreads: {
      eurusd: 1.3,
      gbpusd: 1.5,
      usdjpy: 1.4,
    },
    accessibility: {
      minDeposit: 1,
      depositMethods: [
        'Bank Transfer',
        'Credit/Debit Card',
        'Skrill',
        'Neteller',
        'WebMoney',
        'Bitcoin',
        'Ethereum',
      ],
      withdrawalMethods: [
        'Bank Transfer',
        'Credit/Debit Card',
        'Skrill',
        'Neteller',
        'WebMoney',
        'Bitcoin',
        'Ethereum',
      ],
      customerSupport: [
        '24/7 Live Chat',
        'Phone',
        'Email',
      ],
    },
    brokerType: 'ECN',
    platforms: [
      'MT4',
      'MT5',
      'WebTrader',
    ],
    countries: [
      'Global',
      'Global',
    ],
    features: [
      'EA Support',
      'API Access',
      'Islamic Account',
      'Islamic Friendly',
    ],
  },
  {
    id: 'lifefinance',
    name: 'LifeFinance',
    logoUrl: '/broker-logos/lifefinance.png',
    websiteUrl: 'https://lifefinance.com/',
    score: 7.5,
    foundingYear: 2015,
    headquarters: 'Seychelles',
    description: 'LifeFinance is a Seychelles-based forex and CFD broker offering competitive trading conditions and modern trading platforms.',
    rating: 6,
    regulation: [
      'FSA',
    ],
    minDeposit: 100,
    spreads: {
      eurusd: 1.2,
      gbpusd: 1.4,
      usdjpy: 1.3,
    },
    accessibility: {
      minDeposit: 100,
      depositMethods: [
        'Bank Transfer',
        'Credit/Debit Card',
        'Skrill',
        'Neteller',
      ],
      withdrawalMethods: [
        'Bank Transfer',
        'Credit/Debit Card',
        'Skrill',
        'Neteller',
      ],
      customerSupport: [
        '24/5 Live Chat',
        'Phone',
        'Email',
      ],
    },
    brokerType: 'STP',
    platforms: [
      'MT4',
      'MT5',
      'WebTrader',
    ],
    countries: [
      'Global',
    ],
    features: [
      'EA Support',
      'Islamic Account',
      'Islamic Friendly',
    ],
  },
  {
    id: 'fxgt',
    name: 'FXGT',
    logoUrl: '/broker-logos/fxgt.png',
    websiteUrl: 'https://fxgt.com/',
    score: 7.7,
    foundingYear: 2019,
    headquarters: 'Seychelles',
    description: 'FXGT is a Seychelles-based forex and CFD broker offering competitive trading conditions and access to multiple markets.',
    rating: 6,
    regulation: [
      'FSA',
    ],
    minDeposit: 50,
    spreads: {
      eurusd: 1.2,
      gbpusd: 1.4,
      usdjpy: 1.3,
    },
    accessibility: {
      minDeposit: 50,
      depositMethods: [
        'Bank Transfer',
        'Credit/Debit Card',
        'Skrill',
        'Neteller',
        'WebMoney',
      ],
      withdrawalMethods: [
        'Bank Transfer',
        'Credit/Debit Card',
        'Skrill',
        'Neteller',
        'WebMoney',
      ],
      customerSupport: [
        '24/5 Live Chat',
        'Phone',
        'Email',
      ],
    },
    brokerType: 'ECN',
    platforms: [
      'MT4',
      'MT5',
      'WebTrader',
    ],
    countries: [
      'Global',
    ],
    features: [
      'EA Support',
      'API Access',
      'Islamic Account',
      'Islamic Friendly',
    ],
  },
  {
    id: 'traderstrust',
    name: 'TradersTrust',
    logoUrl: '/broker-logos/traderstrust.png',
    websiteUrl: 'https://www.traderstrust.com/',
    score: 8.1,
    foundingYear: 2009,
    headquarters: 'Limassol, Cyprus',
    description: 'TradersTrust (TT) is a forex and CFD broker offering trading services through MetaTrader platforms with competitive trading conditions and regulatory oversight.',
    rating: 7,
    regulation: [
      'CySEC',
    ],
    minDeposit: 100,
    spreads: {
      eurusd: 1,
      gbpusd: 1.2,
      usdjpy: 0.9,
    },
    accessibility: {
      minDeposit: 100,
      depositMethods: [
        'Credit/Debit Card',
        'Bank Transfer',
        'Skrill',
        'Neteller',
        'WebMoney',
        'Crypto',
      ],
      withdrawalMethods: [
        'Credit/Debit Card',
        'Bank Transfer',
        'Skrill',
        'Neteller',
        'WebMoney',
        'Crypto',
      ],
      customerSupport: [
        '24/5 Live Chat',
        'Phone',
        'Email',
        'Ticket System',
      ],
    },
    brokerType: 'Market Maker',
    platforms: [
      'MT4',
      'MT5',
      'WebTrader',
      'Mobile App',
    ],
    countries: [
      'Global',
    ],
    features: [
      'EA Support',
      'Islamic Account',
      'Islamic Friendly',
      'Trading Signals',
    ],
  },
  {
    id: 'freshforex',
    name: 'FreshForex',
    logoUrl: '/broker-logos/freshforex.png',
    websiteUrl: 'https://www.freshforex.com/',
    score: 7.8,
    foundingYear: 2004,
    headquarters: 'Kingstown, St. Vincent and the Grenadines',
    description: 'FreshForex is an international forex and CFD broker offering a wide range of trading instruments, educational resources, and bonus programs for traders of all levels.',
    rating: 4,
    regulation: [
      'FSA SVG',
    ],
    minDeposit: 1,
    spreads: {
      eurusd: 1.2,
      gbpusd: 1.4,
      usdjpy: 1.1,
    },
    accessibility: {
      minDeposit: 1,
      depositMethods: [
        'Credit/Debit Card',
        'Bank Transfer',
        'Skrill',
        'Neteller',
        'WebMoney',
        'Perfect Money',
        'Crypto',
      ],
      withdrawalMethods: [
        'Credit/Debit Card',
        'Bank Transfer',
        'Skrill',
        'Neteller',
        'WebMoney',
        'Perfect Money',
        'Crypto',
      ],
      customerSupport: [
        '24/7 Live Chat',
        'Phone',
        'Email',
        'Ticket System',
      ],
    },
    brokerType: 'Market Maker',
    platforms: [
      'MT4',
      'MT5',
      'WebTrader',
      'Mobile App',
    ],
    countries: [
      'Global',
    ],
    features: [
      'EA Support',
      'Islamic Account',
      'Islamic Friendly',
      'Trading Signals',
    ],
  },
  {
    id: 'windsor-broker',
    name: 'Windsor Brokers',
    logoUrl: '/broker-logos/windsor-broker.png',
    websiteUrl: 'https://www.windsorbrokers.com/',
    score: 8.3,
    foundingYear: 1988,
    headquarters: 'Limassol, Cyprus',
    description: 'Windsor Brokers is a well-established forex and CFD broker with over 30 years of experience, offering trading services through multiple platforms with strong regulatory oversight.',
    rating: 9,
    regulation: [
      'FCA',
      'CySEC',
      'FSA Bahrain',
    ],
    minDeposit: 100,
    spreads: {
      eurusd: 1.8,
      gbpusd: 2,
      usdjpy: 1.6,
    },
    accessibility: {
      minDeposit: 100,
      depositMethods: [
        'Credit/Debit Card',
        'Bank Transfer',
        'Skrill',
        'Neteller',
        'WebMoney',
      ],
      withdrawalMethods: [
        'Credit/Debit Card',
        'Bank Transfer',
        'Skrill',
        'Neteller',
        'WebMoney',
      ],
      customerSupport: [
        '24/5 Live Chat',
        'Phone',
        'Email',
        'Ticket System',
      ],
    },
    brokerType: 'Market Maker',
    platforms: [
      'MT4',
      'Windsor Direct',
      'Mobile App',
    ],
    countries: [
      'Global',
      'Global',
      'Global',
    ],
    features: [
      'EA Support',
      'Islamic Account',
      'Islamic Friendly',
      'Trading Signals',
    ],
  },
  {
    id: 'tastyfx',
    name: 'TastyFX',
    logoUrl: '/broker-logos/tastyfx.png',
    websiteUrl: 'https://www.tastyfx.com/',
    score: 8.9,
    foundingYear: 2021,
    headquarters: 'Chicago, Illinois, USA',
    description: 'TastyFX is a US-based forex and CFD broker that brings the tastytrade approach to trading with competitive pricing and a focus on options education.',
    rating: 9.5,
    regulation: [
      'NFA',
    ],
    minDeposit: 0,
    spreads: {
      eurusd: 0.7,
      gbpusd: 0.9,
      usdjpy: 0.8,
    },
    accessibility: {
      minDeposit: 0,
      depositMethods: [
        'Bank Transfer (ACH)',
        'Wire Transfer',
      ],
      withdrawalMethods: [
        'Bank Transfer (ACH)',
        'Wire Transfer',
      ],
      customerSupport: [
        '24/5 Live Chat',
        'Phone',
        'Email',
        'Support Tickets',
      ],
    },
    brokerType: 'ECN',
    platforms: [
      'tastytrade platform',
      'Desktop',
      'Mobile App',
    ],
    countries: [
      'Global',
    ],
    features: [
      'Trading Signals',
    ],
  },
]

export const summariesMetadata = {
  totalCount: 78,
  generatedAt: '2025-10-15T07:14:19.897Z',
  averageScore: 8.55,
  topRatedBroker: 'Swissquote',
}
