...
export const getStaticProps: GetStaticProps = async () => {
  ...
  // Import the real broker data
  const { brokers: allBrokers } = await import('../data/brokers');

  // Filter to only include authorized brokers
  const authorizedBrokers = filterAuthorizedBrokers(allBrokers);

  return {
    props: {
      brokers: authorizedBrokers.map(broker => ({
        id: broker.id,
        name: broker.name,
        slug: broker.id,
        ...
        min_deposit_currency: '$',
        max_leverage: parseInt(broker.tradingConditions?.maxLeverage?.split(':')[1] || '0'),
        spreads_from: broker.fees?.trading?.averageSpreads?.[0]?.spread || 0,
        commission: broker.fees?.trading?.commissionStructure || 'No commission',
        regulated: broker.security?.regulatedBy?.length > 0,
        regulations: broker.security?.regulatedBy || [],
        platforms: broker.technology?.platforms || [],
        instruments_total: broker.tradableInstruments ?
          Object.values(broker.tradableInstruments).reduce((sum: number, item: any) => sum + (item?.total || 0), 0) : 0,
        pros: broker.pros || [],
        cons: broker.cons || [],
        ...
        features: broker.technology?.platforms || [],
        cta_text: `Visit ${broker.name}`,
        cta_url: broker.websiteUrl
      })),
      totalCount: authorizedBrokers.length,
      categories,
      regulators,
      platforms
    },
